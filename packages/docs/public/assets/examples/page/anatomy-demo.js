...
const frame = demo.querySelector('cs-zoomable-frame');

// Wait for the frame to load
await new Promise((resolve) => frame.addEventListener('load', resolve, { once: true }));
await allDefined(frame.contentDocument);

const page = frame.contentDocument.querySelector('cs-page');
...
function updatePageContent() {
  ...
  const checkedBoxes = fieldset.querySelectorAll('cs-checkbox:state(checked)');
