---
title: Agent Skills
description: Two complementary skills that AI tools load progressively, fetching only the documentation a task needs.
sidebar:
  badge:
    text: Recommended
    variant: tip
...
Cornerstone publishes **two complementary Agent Skills** that are designed to work together. If your tool supports loading more than one skill, install both — they cross-reference each other, and the combination covers the full range of design and component work an AI tool will hit on a real task.

| Skill               | Answers the question                             | Reach for it when you're                                                          |
| ------------------- | ------------------------------------------------ | --------------------------------------------------------------------------------- |
| `cornerstone`        | "What does this component do?"                   | Working with a specific component's properties, slots, events, or framework setup |
| `cornerstone-design` | "How do I build a good-looking page with these?" | Laying out a page (`<cs-page>`), theming on-brand, or composing a polished UI     |

## Which Skill Do I Need?

The simplest way to decide:

- **Reaching for a specific component** (a button, an input, a dialog) and you need its exact API? That's the **`cornerstone`** skill.
- **Starting from a goal rather than a component** ("build a landing page", "make this match our brand", "lay out a dashboard")? That's the **`cornerstone-design`** skill.

In practice the two overlap on real tasks. "Build a settings page" starts in `cornerstone-design` (layout, spacing, theming), then leans on `cornerstone` for the exact API of each `cs-input` and `cs-switch` along the way. That's why they cross-reference each other, and why installing both gives AI tools the most complete picture.

If you can only install one, pick `cornerstone` for component-heavy work and `cornerstone-design` for design-heavy work. Most real tasks benefit from both.
...
When working with AI coding assistants like Claude Code, Cursor, or other tools that support Agent Skills, these skills give the AI deep context about Cornerstone. They cover both the component APIs and how to design with them, which leads to more accurate code suggestions, fewer hallucinations, and output that actually looks designed.
...
Both skills are kept in sync with the library by tooling. The `cornerstone` skill is generated from the Custom Elements Manifest on every release, so it matches the current component API exactly. The `cornerstone-design` skill is hand-authored but verified — every component tag, attribute, slot, and CSS custom property it cites is cross-checked against the library on every build, so it can't claim something the components don't actually do.

## How to Use Them

Both skills ship in every Cornerstone build and are available in your `node_modules` directory after installing via npm:

```
node_modules/@cruglobal/cornerstone-components/dist/unbundled/skills/cornerstone/
node_modules/@cruglobal/cornerstone-components/dist/unbundled/skills/cornerstone-design/
```
...
npx skills add ./node_modules/@cruglobal/cornerstone-components/dist/unbundled/skills/cornerstone
npx skills add ./node_modules/@cruglobal/cornerstone-components/dist/unbundled/skills/cornerstone-design

# Uninstall
npx skills remove cornerstone
...
```
@node_modules/@cruglobal/cornerstone-components/dist/unbundled/skills/cornerstone/
@node_modules/@cruglobal/cornerstone-components/dist/unbundled/skills/cornerstone/references/components/button.md
@node_modules/@cruglobal/cornerstone-components/dist/unbundled/skills/cornerstone-design/references/layouts-page.md
```
...
The skills follow the [Agent Skills specification](https://agentskills.io/), so any tool that supports the spec should be able to use them. We've tested in Claude Code and Cursor. If your tool can load a directory of markdown references on demand, point it at the same `node_modules/@cruglobal/cornerstone-components/dist/unbundled/skills/` directories shown above.

<div class="cs-flank:end">
  <h2>The Component Skill</h2>
  <code>cornerstone</code>
</div>

The `cornerstone` skill is the **component reference**. It's generated from the library on every build, so it always matches the current API. Use it when you need to know what a component does: its properties, slots, events, CSS parts, or framework integration.
...
└── references/
    ├── layouts-page.md       # Full-page layouts with <cs-page> + a <cs-page>-specific structural checklist
...
- **Layout:** when and how to use `<cs-page>` for full-page layouts (with an explicit landing-page vs. app-shell branch to head off the most common sidebar mistake), and how to compose sections, widgets, and forms with layout utilities when `<cs-page>` isn't the right fit
- **Theming:** choosing themes and palettes, light and dark mode, matching a brand color, and customizing with `--cs-*` tokens
...
- **Self-check passes:** a **structural** final pass in the main skill (markup, slot decisions, rule compliance) plus a **visual polish** checklist in composition.md (rhythm, hierarchy, contrast on surfaces) and a `<cs-page>`-specific structural checklist in layouts-page.md
- **Recipes:** best-practice starting points for common screens

## Agent Skills vs llms.txt

Cornerstone provides both an [llms.txt file](/ai/llms) and [Agent Skills](https://agentskills.io/). Here's when to use each:
