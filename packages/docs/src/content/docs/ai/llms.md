---
title: LLMs
description: A single text file holding the full API reference for every component, for any tool that takes pasted context.
---
...
Cornerstone publishes an `llms.txt` file that provides AI tools with structured information about our components, including their APIs, properties, events, methods, slots, and CSS custom properties.
...
When working with AI coding assistants like Claude, ChatGPT, Copilot, or Cursor, you can reference the llms.txt file to give the AI context about Cornerstone components. This can lead to more accurate code suggestions and fewer hallucinations when the AI generates Cornerstone code.

## Accessing the File

The llms.txt file is available in every Cornerstone build at:

```
/dist/llms.txt
/dist/bundled/llms.txt
```

You can also find it in your `node_modules` directory if you've installed Cornerstone via npm:

```
node_modules/@cruglobal/cornerstone-components/dist/unbundled/llms.txt
```
...
If you're using [Claude Projects](https://www.anthropic.com/news/projects), you can add the llms.txt URL to your project knowledge. Claude will use this context when helping you write Cornerstone code.
...
- An overview of Cornerstone and its capabilities
...
Cornerstone provides both an [Agent Skill](https://agentskills.io/) and an [llms.txt file](/ai/llms). Here's when to use each:
