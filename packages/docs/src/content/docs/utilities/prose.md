---
title: Prose
description: The cs-prose utility applies hierarchical, asymmetric typographic rhythm to long-form content like documentation, articles, and marketing copy.
tags: styleUtilities
synonyms:
  - prose
  - longform
...
Wrap a block of content in `cs-prose` to apply a hierarchical, asymmetric typographic rhythm: generous space above headings, tighter space below, more breathing room around major non-text blocks, and a true section divider for `<hr>`. Spacing is em-based and scales with `cs-font-size-*` utilities.

Reach for it on documentation, blog posts, articles, or marketing copy. Element styling (color, font, borders) still comes from [native styles](/utilities/native/); `cs-prose` only adjusts rhythm, type scale, and the reading column.

## Using Prose

Wrap your long-form content in any block element with the `cs-prose` class.
...
By default, content is constrained to a comfortable reading column of `65ch`. Override `--cs-prose-line-length` on the container — or set `max-inline-size` directly — to widen, narrow, or remove the constraint.
...
Lists get a small breath between multi-line items. Quiet markers and bold `<dt>` terms come from [native styles](/utilities/native/).
...
Inline elements you'd reach for in long-form writing — `<kbd>`, `<mark>`, `<sub>`/`<sup>`, `<abbr>` — work as expected inside a prose container, styled by [native styles](/utilities/native/).
...
develop in HC-110, dilution B</code></pre>
  ...
  <cs-callout variant="brand">
    <cs-icon slot="icon" name="lightbulb"></cs-icon>
...
Apply any [`cs-font-size-*`](/utilities/text/#font-size) utility to a `cs-prose` container and text, headings, and rhythm scale together. No size variants required.

```html {.example}
<div class="cs-cluster cs-align-items-start" style="gap: var(--cs-space-l);">
  <article class="cs-prose" style="--cs-prose-line-length: 28ch;">
    <h3>Default size</h3>
  ...
  <article class="cs-prose cs-font-size-s" style="--cs-prose-line-length: 28ch;">
    <h3>With cs-font-size-s</h3>
    <p>A quiet morning is the rarest hour of the day — claim it before the world wakes up.</p>
    <ul>
      <li>One cup, one book, one window.</li>
      <li>No notifications until the second pour.</li>
...
Set `--cs-prose-rhythm-scale` on the prose container to multiply every margin in the system. Values below `1` tighten the rhythm; values above loosen it. Type sizes are unaffected.
...
<div class="cs-cluster cs-align-items-start" style="gap: var(--cs-space-l);">
  ...
  <article class="cs-prose" style="--cs-prose-line-length: 28ch; --cs-prose-rhythm-scale: 0.6;">
...
The `cs-prose` class and its element rules sit at `0,0,0` specificity, so any utility class you apply alongside — `cs-heading-m`, `cs-cluster`, `cs-text-center`, and so on — wins automatically. The same goes for plain element rules in your own stylesheet, no `!important` or specificity tricks required.

```css
/* Wins against cs-prose's `h2 { font-size: 2em }` */
h2.release-header {
...
Color flows from your theme's [color tokens](/tokens/color/), so prose follows dark mode and theme changes automatically. To recolor an element inside prose, use a descendant selector on your container.

```css
.changelog.cs-prose a {
...
Apply `cs-not-prose` to any element inside a `cs-prose` container to disable prose rhythm for that element and its descendants. Other utilities — `cs-cluster`, `cs-stack`, `cs-font-size-*` — keep working in the opt-out subtree.

```html {.example}
<article class="cs-prose cs-font-size-s">
  <h3>Ready when you are</h3>
  <p>
    The content in this section follows prose rhythm and adopts a smaller font size. The callout below is given
    <code>cs-not-prose</code>, so its spacing and font sizing revert to element defaults.
  </p>

  <cs-callout class="cs-not-prose" variant="warning">
    <cs-icon slot="icon" name="ink_highlighter"></cs-icon>
    <div class="cs-stack cs-gap-s">
      <h4>Leave it to the prose</h4>
      <p>
        This callout and its child elements are exempt from <code>cs-prose</code> rules, thanks to
        <code>cs-not-prose</code>.
      </p>
    </div>
  </cs-callout>
