---
title: Localization
description: Discover how to localize Cornerstone with minimal effort.
synonyms:
  - i18n
...
Cornerstone ships with a number of translations, one file per language in `src/translations`. The default is English (US), which also serves as the fallback locale. As such, you do not need to import the English translation.
...
Cornerstone uses English as a fallback to provide a better experience than rendering nothing or throwing an error.

### Submitting New Translations or Improvements

Translations live in `src/translations`, one file per language. Add or correct one there and open a pull request.

Regional translations are welcome. If a German translation (`de`) exists, a German (Switzerland) (`de-CH`) translation on top of it is perfectly reasonable.

:::info
Cornerstone provides a localization mechanism for component internals. This is not designed to be used as localization tool for your entire application. You should use a more appropriate tool such as [i18next](https://www.i18next.com/) if you need to localize content in your app.
:::
...
<html lang="es">
  ...
  <body>
    <cs-button><!-- Spanish --></cs-button>
    <cs-button lang="ru"><!-- Russian --></cs-button>
  </body>
</html>
```
...
<html lang="es">
  ...
  <body>
    <div lang="ru">
      <cs-button><!-- still in Spanish --></cs-button>
    </div>
  </body>
</html>
```
...
```ts
import { registerTranslation } from 'path/to/cornerstone.js';
import type { Translation } from 'path/to/cornerstone.js';
