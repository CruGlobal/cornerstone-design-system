...
<style>
  /* These widths exist to overflow the scroller — that is the whole demo — so they are deliberately
     larger than any container and have no meaningful token. Defined once here rather than repeated
     inline, which is the part rule 8 is about. */
  .wide-content {
    inline-size: 1200px;
  ...
  .tall-scroller {
    max-block-size: calc(var(--cs-space-5xl) * 3.75);
  }
</style>

```html {.example}
<cs-scroller id="scroller__overview">
  <table>
    <tr>
      <th>Party Role</th>
      <th>Combat Style</th>
...
The scroller component automatically provides a scrollable container for any content that exceeds the available space. Add your content as children of the `<cs-scroller>` element, and it handles the rest.

```html {.example}
<cs-scroller>
  <div class="wide-content">
    <h3>Superhero Team Roles Guide</h3>
    <div class="cs-grid cs-gap-l" style="--min-column-size: calc(var(--cs-space-4xl) * 4)">
      <div>
        <h4>Team Leaders</h4>
        <p>
...
```html {.example}
<cs-scroller orientation="vertical" class="tall-scroller">
  <p>
