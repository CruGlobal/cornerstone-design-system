...
const shipped = new Set(
  readdirSync(join(componentsRoot, "src", "components"), {
    withFileTypes: true,
  })
...
if (unknown.size === 0) {
  console.log(
    `\n${files.length} pages checked (${drafts} draft${
      drafts === 1 ? "" : "s"
...
console.error(
  "An unknown custom element renders as an inert box, and `whenDefined` on one never resolves.\n"
);
process.exit(1);
