...
const translation: Translation = {
  ...
  jumpBackwardX: (count) => {
    if (count === 1) {
      return 'Přejít o 1 stránku zpět';
    }
    if (count >= 2 && count <= 4) {
      return `Přejít o ${count} stránky zpět`;
    }
    return `Přejít o ${count} stránek zpět`;
  },
  jumpForwardX: (count) => {
    if (count === 1) {
      return 'Přejít o 1 stránku vpřed';
    }
    if (count >= 2 && count <= 4) {
      return `Přejít o ${count} stránky vpřed`;
    }
  ...
  numCharactersRemaining: (num) => {
    if (num === 1) {
      return '1 zbývající znak';
    }
    if (num >= 2 && num <= 4) {
      return `${num} zbývající znaky`;
    }
  ...
  numRowsCopied: (num) => {
    if (num === 1) {
      return 'Byl zkopírován 1 řádek';
    }
    if (num >= 2 && num <= 4) {
      return `Byly zkopírovány ${num} řádky`;
    }
    return `Bylo zkopírováno ${num} řádků`;
  },
  numRowsSelected: (num) => {
    if (num === 1) {
      return 'Je vybrán 1 řádek';
    }
    if (num >= 2 && num <= 4) {
      return `Jsou vybrány ${num} řádky`;
    }
  ...
  rangeTooLong: (max) => {
    if (max === 1) {
      return 'Vyberte rozsah nejvýše 1 den';
    }
    if (max >= 2 && max <= 4) {
      return `Vyberte rozsah nejvýše ${max} dny`;
    }
    return `Vyberte rozsah nejvýše ${max} dnů`;
  },
  rangeTooShort: (min) => {
    if (min === 1) {
      return 'Vyberte rozsah dlouhý alespoň 1 den';
    }
    if (min >= 2 && min <= 4) {
      return `Vyberte rozsah dlouhý alespoň ${min} dny`;
    }
