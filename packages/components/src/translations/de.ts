...
const translation: Translation = {
  ...
  rangeTooLong: (max) => {
    if (max === 1) {
      return 'Wählen Sie einen Zeitraum von höchstens 1 Tag';
    }
    return `Wählen Sie einen Zeitraum von höchstens ${max} Tagen`;
  },
  rangeTooShort: (min) => {
    if (min === 1) {
      return 'Wählen Sie einen Zeitraum von mindestens 1 Tag';
    }
