...
import type { CornerstoneFormControl } from '../cornerstone-form-associated-element.js';
...
type CreateControlFn = () => Promise<CornerstoneFormControl>;

/** Runs a set of generic tests for Cornerstone form controls */
export function runFormControlBaseTests<T extends CornerstoneFormControl = CornerstoneFormControl>(
  tagNameOrConfig:
    | string
    | {
        tagName: string;
        init?: (control: T) => void;
        /** Distinguishes several configurations of one tag. Omit when there is only one. */
        variantName?: string;
  ...
  const displayName =
    isStringArg || !tagNameOrConfig.variantName //
      ? tagName
  ...
  const formValue = isStringArg ? undefined : tagNameOrConfig.formValue;
  runResetTests(tagName, displayName, formValue, init);
}

/**
 * How a control's restorable value is exercised. Reset is otherwise untestable generically: the suite has to
 * know a value that differs from the control's default, and only the control knows what is valid for it.
 */
export interface FormValueConfig {
  /** The property carrying the restorable value. `checked` for boolean controls. Defaults to `value`. */
  property?: 'value' | 'checked';

  /**
   * A value different from the control's default, assigned to dirty it before reset. Optional when
   * `property` is `checked`, where the opposite of the default is the only other option.
   */
  dirtyValue?: unknown;
}

//
// Applicable for all Cornerstone form controls. This function checks the behavior of:
//   - `.validity`
//   - `.validationMessage`,
//   - `.checkValidity()`
//   - `.reportValidity()`
...
function runAllValidityTests(
  tagName: string, //
  displayName: string,
  renderControl: (fixture: typeof hydratedFixture | typeof clientFixture) => () => Promise<CornerstoneFormControl>,
) {
  ...
  describe(`Form validity base test for ${displayName}`, () => {
    for (const fixture of fixtures) {
      describe(`with ${fixture.type} rendering`, () => {
        ...
        it('should not emit an `cs-invalid` event when `.checkValidity()` is called while valid', async () => {
          const control = await createControl();
          const emittedEvents = await checkEventEmissions(control, 'cs-invalid', () => control.checkValidity());
          expect(emittedEvents.length).to.equal(0);
        });

        it('should not emit an `cs-invalid` event when `.reportValidity()` is called while valid', async () => {
          const control = await createControl();
          const emittedEvents = await checkEventEmissions(control, 'cs-invalid', () => control.reportValidity());
          expect(emittedEvents.length).to.equal(0);
        });

        // TODO: As soon as `CsRadioGroup` has a property `disabled` this
        // condition can be removed
        if (tagName !== 'cs-radio-group') {
          it('should not emit an `cs-invalid` event when `.checkValidity()` is called in custom error case while disabled', async () => {
            const control = await createControl();
          ...
          it('should not emit an `cs-invalid` event when `.reportValidity()` is called in custom error case while disabled', async () => {
            const control = await createControl();
            control.setCustomValidity('error');
            control.disabled = true;
            await control.updateComplete;
...
//
//  Special tests for <cs-button type="button">
//
function runSpecialTests_slButtonOfTypeButton(createControl: CreateControlFn) {
  ...
  it('should emit an `cs-invalid` event when `.checkValidity()` is called in custom error case, and not disabled', async () => {
    const control = await createControl();
    control.setCustomValidity('error');
    control.disabled = false;
    await control.updateComplete;
  ...
  it('should emit an `cs-invalid` event when `.reportValidity()` is called in custom error case, and not disabled', async () => {
    const control = await createControl();
    control.setCustomValidity('error');
    control.disabled = false;
    await control.updateComplete;
...
//
// Special tests for <cs-button href="...">
//
function runSpecialTests_slButtonWithHref(createControl: CreateControlFn) {
  ...
  it('should emit an `cs-invalid` event when `.checkValidity()` is called in custom error case', async () => {
    const control = await createControl();
    control.setCustomValidity('error');
    await control.updateComplete;
    const emittedEvents = await checkEventEmissions(control, 'cs-invalid', () => control.checkValidity());
  ...
  it('should emit an `cs-invalid` event when `.reportValidity()` is called in custom error case', async () => {
    const control = await createControl();
    control.setCustomValidity('error');
    await control.updateComplete;
    const emittedEvents = await checkEventEmissions(control, 'cs-invalid', () => control.reportValidity());
...
// Creates a testable Cornerstone form control instance. Takes in a fixture function for SSR vs Client rendering
function createFormControl<T extends CornerstoneFormControl = CornerstoneFormControl>(fixtureFn: FixtureFunction<T>) {
...
function checkEventEmissions(control: CornerstoneFormControl, eventType: string, action: () => void): Promise<Event[]> {
...
// Component `cs-button` behaves quite different to the other components. To keep things simple we use simple conditions
// here. `cs-button` might stay the only component in Cornerstone core behaves that way, so we just hard code it here.
function getMode(control: CornerstoneFormControl) {
  if (
    control.localName === 'cs-button' && //
    'href' in control &&
    'type' in control &&
    control.type === 'button' &&
    !control.href
  ...
  // <cs-button href="...">
  if (control.localName === 'cs-button' && 'href' in control && !!control.href) {
...
//
// Reset and default-value behaviour, which the validity tests above do not touch. Applicable to any control
// that carries a value a user can change — so not `cs-button`, which has none, and not `cs-radio`, whose value
// and default are owned by its `cs-radio-group`.
//
function runResetTests(
  tagName: string,
  displayName: string,
  config: FormValueConfig | undefined,
  init: ((control: CornerstoneFormControl) => void) | null,
) {
  const property = config?.property ?? 'value';
  const defaultProperty = property === 'checked' ? 'defaultChecked' : 'defaultValue';

  describe(`Form reset base test for ${displayName}`, () => {
    for (const fixture of fixtures) {
      describe(`with ${fixture.type} rendering`, () => {
        const renderInForm = async () => {
          const tag = unsafeStatic(tagName);
          const form = await fixture<HTMLFormElement>(staticHTML`<form><${tag}></${tag}></form>`);
          const control = form.querySelector(tagName) as CornerstoneFormControl;
          await customElements.whenDefined(tagName);
          await control.updateComplete;

          // A control whose value has to match a child — `cs-select` and its options — supplies them here.
          if (init) {
            init(control);
            await control.updateComplete;
            await aTimeout(0);
        ...
        if (!config) {
          // Recorded rather than silently absent. A control that owns no value owns no default: `cs-button` has
          // no value at all, and `cs-radio`'s value and default belong to its `cs-radio-group`. Asserting a
          // default property here would contradict that rule rather than test it.
          it.skip('should restore its default on form reset (owns no value)', () => {});
          return;
        }

        it(`should expose \`${defaultProperty}\``, async () => {
          const { control } = await renderInForm();
          expect(defaultProperty in control).to.equal(true);
        });

        it('should restore its default on form reset', async () => {
          const { form, control } = await renderInForm();
          const initial = (control as unknown as Record<string, unknown>)[property];
          const dirtyValue = 'dirtyValue' in config ? config.dirtyValue : !initial;

          // Guards the test itself: a dirty value equal to the default would make the assertion vacuous.
          expect(dirtyValue).to.not.equal(initial);

          (control as unknown as Record<string, unknown>)[property] = dirtyValue;
          await control.updateComplete;
          expect((control as unknown as Record<string, unknown>)[property]).to.not.equal(initial);

          form.reset();
          await control.updateComplete;
          expect((control as unknown as Record<string, unknown>)[property]).to.equal(initial);
        });
      });
    }
  });
