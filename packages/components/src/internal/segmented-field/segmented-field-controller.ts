 ...
 * input. It is the generic skeleton used by `<cs-date-input>` and `<cs-time-input>`, and is intentionally agnostic
 ...
 *
 * Why a controller instead of a class mixin: Cornerstone form controls already extend
 * `CornerstoneFormAssociatedElement`, and date/time pickers need different validators on top of that base. A
...
export class SegmentedFieldController implements ReactiveController {
  ...
  moveFocus(from: HTMLElement, direction: -1 | 1, options?: FocusOptions): void {
    ...
    if (next) {
      next.focus({ preventScroll: true, ...options });
    }
  }
