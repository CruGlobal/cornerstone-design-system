...
// CornerstoneElement is deliberately not imported here: it shouldn't exist in the final bundle.

const getMetadata = () => readFile({ path: '../../dist/unbundled/custom-elements.json' }) as unknown as Promise<string>;
...
before(async () => {
  ...
  relevantMetadata.forEach(({ tagName, path }) => {
    it(`Should not register any components: ${tagName}`, async () => {
      await import('../../dist/bundled/' + path);
