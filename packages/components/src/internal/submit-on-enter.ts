import type CsButton from '../components/button/button.js';
import type { CornerstoneFormAssociatedElement } from './cornerstone-form-associated-element.js';
...
export function submitForm(el: HTMLElement | CornerstoneFormAssociatedElement) {
  ...
  const button = formElements.find((el: HTMLButtonElement) => el.type === 'submit' && !el.matches(':disabled')) as
    undefined | HTMLButtonElement | CsButton;
  ...
  if (['input', 'button'].includes(button.localName)) {
    ...
    // requestSubmit() wont work with `<cs-button>`, so trigger a manual click.
    button.click();
  }
}
