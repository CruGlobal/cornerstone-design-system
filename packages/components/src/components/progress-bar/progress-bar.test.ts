...
import type CsProgressBar from './progress-bar.js';

describe('<cs-progress-bar>', () => {
  for (const fixture of fixtures) {
    describe(`with "${fixture.type}" rendering`, () => {
      describe('accessibility', () => {
        it('should be accessible with default properties', async () => {
          const el = await fixture<CsProgressBar>(html`<cs-progress-bar value="25"></cs-progress-bar>`);
          await expect(el).to.be.accessible();
        });

        it('should be accessible when indeterminate', async () => {
          const el = await fixture<CsProgressBar>(html`<cs-progress-bar indeterminate></cs-progress-bar>`);
          await expect(el).to.be.accessible();
        });

        it('should be accessible with a custom label', async () => {
          const el = await fixture<CsProgressBar>(
            html`<cs-progress-bar value="50" label="Loading files"></cs-progress-bar>`,
          );
          await expect(el).to.be.accessible();
        });
      });
      ...
      describe('properties', () => {
        it('should have default property values', async () => {
          const el = await fixture<CsProgressBar>(html`<cs-progress-bar></cs-progress-bar>`);
        ...
        it('should reflect the value attribute', async () => {
          const el = await fixture<CsProgressBar>(html`<cs-progress-bar value="50"></cs-progress-bar>`);
          expect(el.value).to.equal(50);
          expect(el.getAttribute('value')).to.equal('50');
        });

        it('should set aria-valuenow from value', async () => {
          const el = await fixture<CsProgressBar>(html`<cs-progress-bar value="75"></cs-progress-bar>`);
          const base = el.shadowRoot!.querySelector('[part~="progress-bar"]')!;
        ...
        it('should clamp value between 0 and 100', async () => {
          const el = await fixture<CsProgressBar>(html`<cs-progress-bar value="150"></cs-progress-bar>`);
          await new Promise(requestAnimationFrame);
        ...
        it('should use custom label for aria-label when provided', async () => {
          const el = await fixture<CsProgressBar>(
            html`<cs-progress-bar value="50" label="Uploading"></cs-progress-bar>`,
          );
          const base = el.shadowRoot!.querySelector('[part~="progress-bar"]')!;
        ...
        it('should hide the label slot when indeterminate', async () => {
          const el = await fixture<CsProgressBar>(html`<cs-progress-bar indeterminate>50%</cs-progress-bar>`);
      ...
      describe('slots', () => {
        it('should render the default slot for the label', async () => {
          const el = await fixture<CsProgressBar>(html`<cs-progress-bar value="50">50%</cs-progress-bar>`);
