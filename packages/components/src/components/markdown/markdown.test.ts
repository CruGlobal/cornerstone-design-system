...
import type CsMarkdown from './markdown.js';

/**
 * Creates a `<cs-markdown>` element with a `<script type="text/markdown">` child containing the given markdown source.
 * We build this imperatively because Lit's html`` tag doesn't allow `<script>` elements inside templates.
 */
function createMarkdownElement(markdown: string, attrs: Record<string, string> = {}): CsMarkdown {
  const el = document.createElement('cs-markdown');
 ...
 */
async function renderMarkdown(markdown: string, attrs: Record<string, string> = {}): Promise<CsMarkdown> {
...
describe('<cs-markdown>', () => {
  for (const fixture of fixtures) {
    describe(`with "${fixture.type}" rendering`, () => {
      describe('properties', () => {
        it('should have correct default property values', async () => {
          const el = await fixture<CsMarkdown>(html`<cs-markdown></cs-markdown>`);
          expect(el.tabSize).to.equal(4);
        });

        it('should accept the tab-size attribute', async () => {
          const el = await fixture<CsMarkdown>(html`<cs-markdown tab-size="2"></cs-markdown>`);
        ...
        it('should return the same Marked instance from getMarked() and the marked property', async () => {
          const el = await fixture<CsMarkdown>(html`<cs-markdown></cs-markdown>`);
          const Ctor = el.constructor as typeof CsMarkdown;
          expect(el.marked).to.equal(Ctor.getMarked());
        });

        it('should share the same Marked instance across multiple elements', async () => {
          const el1 = await fixture<CsMarkdown>(html`<cs-markdown></cs-markdown>`);
          const el2 = await fixture<CsMarkdown>(html`<cs-markdown></cs-markdown>`);
