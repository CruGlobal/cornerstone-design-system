...
import { CsSlideChangeEvent } from '../../events/slide-change.js';
...
import type CsCarouselItem from '../carousel-item/carousel-item.js';
import '../icon/icon.js';
 ...
 */
@customElement('cs-carousel')
export default class CsCarousel extends CornerstoneElement {
  static css = styles;
  ...
  protected willUpdate(changedProperties: PropertyValueMap<CsCarousel> | Map<PropertyKey, unknown>): void {
  ...
  private synchronizeSlides() {
    const io = new IntersectionObserver(
      (entries) => {
        ...
        const slideIndex = slidesWithClones.indexOf(firstIntersecting.target as CsCarouselItem);
  ...
  private isCarouselItem(node: Node): node is CsCarouselItem {
    return node instanceof Element && node.tagName.toLowerCase() === 'cs-carousel-item';
  }
  ...
  addSlide(slide: CsCarouselItem) {
    if (!this.isCarouselItem(slide)) {
      throw new TypeError('addSlide() expects a <cs-carousel-item>.');
    }
