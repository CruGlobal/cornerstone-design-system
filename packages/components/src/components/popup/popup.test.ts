...
import type CsPopup from './popup.js';
    ...
    describe(`with "${fixture.type}" rendering`, () => {
      describe('accessibility', () => {
        it('should pass accessibility tests', async () => {
          const el = await fixture<CsPopup>(
            html`<cs-popup
              ><div slot="anchor">Anchor</div>
              <div>Popup content</div></cs-popup
            >`,
          );
      ...
      describe('properties', () => {
        it('should have correct default property values', async () => {
          const el = await fixture<CsPopup>(html`<cs-popup></cs-popup>`);
        ...
        it('should set the anchor property to an element reference', async () => {
          const el = await fixture<CsPopup>(html`
            <div>
          ...
          const popup = el.querySelector<CsPopup>('cs-popup')!;
        ...
        it('should set the anchor property to a string id', async () => {
          const el = await fixture<CsPopup>(html`
            <div>
              <span id="my-anchor">Anchor</span>
              <cs-popup anchor="my-anchor"></cs-popup>
            </div>
          `);
        ...
        it('should set the arrow property', async () => {
          const el = await fixture<CsPopup>(html`<cs-popup arrow></cs-popup>`);
          expect(el.arrow).to.be.true;
        });

        it('should set the flip property', async () => {
          const el = await fixture<CsPopup>(html`<cs-popup flip></cs-popup>`);
          expect(el.flip).to.be.true;
        });

        it('should set the shift property', async () => {
          const el = await fixture<CsPopup>(html`<cs-popup shift></cs-popup>`);
          expect(el.shift).to.be.true;
        });

        it('should set the distance property', async () => {
          const el = await fixture<CsPopup>(html`<cs-popup distance="10"></cs-popup>`);
          expect(el.distance).to.equal(10);
        });

        it('should set the skidding property', async () => {
          const el = await fixture<CsPopup>(html`<cs-popup skidding="5"></cs-popup>`);
          expect(el.skidding).to.equal(5);
        });

        it('should set the hover-bridge property', async () => {
          const el = await fixture<CsPopup>(html`<cs-popup hover-bridge></cs-popup>`);
          expect(el.hoverBridge).to.be.true;
        });
      });

      describe('events', () => {
        it('should emit cs-reposition when active and repositioned', async () => {
          const el = await fixture<CsPopup>(html`
            <div>
              <span id="anchor" style="display: inline-block; width: 50px; height: 50px;">Anchor</span>
              <cs-popup anchor="anchor">
          ...
          popup.addEventListener('cs-reposition', () => {
      ...
      describe('behavior', () => {
        ...
        it('should set data-current-placement when active with a valid anchor', async () => {
          const el = await fixture<CsPopup>(html`
            <div>
              <span id="anchor" style="display: inline-block; width: 50px; height: 50px;">Anchor</span>
              <cs-popup anchor="anchor" placement="bottom">
                <div style="width: 100px; height: 100px;">Popup content</div>
              </cs-popup>
            </div>
          `);
