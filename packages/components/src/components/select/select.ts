...
export default class CsSelect extends CornerstoneFormAssociatedElement {
  ...
  private cachedOptions: CsOption[] | null = null;
  ...
  @state() displayLabel = '';
  @state() currentOption: CsOption;
  @state() selectedOptions: CsOption[] = [];
  /** @internal */
  ...
  set value(val: string | string[] | null) {
    ...
    if ((val as unknown) instanceof FormData) {
   ...
   */
  @property({ attribute: false }) getTag: (option: CsOption, index: number) => TemplateResult | string | HTMLElement = (
    option,
  ) => {
    return html`
      <cs-tag
        ...
        @cs-remove=${(event: CsRemoveEvent) => this.handleTagRemove(event, option)}
      >
        ${option.label}
      </cs-tag>
    `;
  ...
  private handleDocumentKeyDown = (event: KeyboardEvent) => {
    ...
    // Ignore presses when the target is a button (e.g. the remove button in `<cs-tag>`)
  ...
  private handleTagRemove(event: CsRemoveEvent, directOption?: CsOption) {
    event.stopPropagation();
    ...
    if (!option) {
      const tagElement = (event.target as Element).closest<HTMLElement>('cs-tag[data-value]');
  ...
  private getAllOptions() {
    ...
    this.cachedOptions = [...this.querySelectorAll<CsOption>('cs-option')];
    return this.cachedOptions;
  }

  // Gets the first `<cs-option>` element
  private getFirstOption() {
    return this.querySelector<CsOption>('cs-option');
  }

  // Sets the current option, which is the option the user is currently interacting with (e.g. via keyboard). Only one
  // option may be "current" at a time.
  private setCurrentOption(option: CsOption | null) {
    const allOptions = this.getAllOptions();
  ...
  private setSelectedOptions(option: CsOption | CsOption[]) {
    const allOptions = this.getAllOptions();
  ...
  private toggleOptionSelection(option: CsOption, force?: boolean) {
            ...
            >
              ...
              ${
                this.multiple && this.hasUpdated
                  ? html`<div part="tags" class="tags" @cs-remove=${this.handleTagRemove}>${this.tags}</div>`
                  : ''
              }
