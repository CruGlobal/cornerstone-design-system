...
import type CsToastItem from '../toast-item/toast-item.js';
...
export interface ToastIconOptions {
  ...
  library?: string;
  /** The family of icons to choose from (e.g., 'sharp', 'outlined', 'rounded'). */
  family?: string;
  /** The icon variant (e.g., 'regular', 'fill'). */
  variant?: string;
}
...
export interface ToastCreateOptions {
  ...
  variant?: 'brand' | 'highlight' | 'information' | 'success' | 'warning' | 'danger' | 'neutral';
...
export default class CsToast extends CornerstoneElement {
  static css = styles;

  private activatedToastItems = new WeakSet<CsToastItem>();
  ...
  @property({ reflect: true }) placement:
    'top-start' | 'top-center' | 'top-end' | 'bottom-start' | 'bottom-center' | 'bottom-end' = 'top-end';
  ...
  private getToastItems(): CsToastItem[] {
    return [...this.querySelectorAll<CsToastItem>(':scope > cs-toast-item')];
  }
  ...
  private async animatePositions() {
    ...
    for (const item of this.getToastItems()) {
      ...
      if (!oldRect) {
        continue;
      } // New item, skip FLIP (it has its own enter animation)
   ...
   * item element.
   */
  async create(message: string, options?: ToastCreateOptions): Promise<CsToastItem> {
