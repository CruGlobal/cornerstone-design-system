...
import type CsToast from './toast.js';
    ...
    describe(`with "${fixture.type}" rendering`, () => {
      describe('accessibility', () => {
        it('should pass accessibility tests', async () => {
          const el = await fixture<CsToast>(html`<cs-toast></cs-toast>`);
      ...
      describe('placement', () => {
        ...
        for (const placement of placements) {
          it(`should support ${placement} placement`, async () => {
            const el = await fixture<CsToast>(html`<cs-toast placement="${placement}"></cs-toast>`);
            await el.updateComplete;
        ...
        it('should keep centered placements within a narrow viewport', async () => {
          ...
          try {
            ...
            for (const placement of ['top-center', 'bottom-center'] as const) {
              ...
              const itemRect = el.querySelector('cs-toast-item')!.getBoundingClientRect();
      ...
      describe('create() method', () => {
        ...
        it('should create a toast item with icon as string', async () => {
          ...
          const icon = toastItem.querySelector('cs-icon[slot="icon"]');
        ...
        it('should create a toast item with icon as object', async () => {
          ...
          const icon = toastItem.querySelector('cs-icon[slot="icon"]');
          expect(icon).to.exist;
          expect(icon?.getAttribute('name')).to.equal('notifications');
          expect(icon?.getAttribute('family')).to.equal('rounded');
          expect(icon?.getAttribute('variant')).to.equal('fill');
        ...
        it('should prepend new toast items to the stack', async () => {
          ...
          const toastItems = el.querySelectorAll('cs-toast-item');
      ...
      describe('accessibility announcements', () => {
        it('should mount a visually hidden live region container in the document body', async () => {
          await fixture<CsToast>(html`<cs-toast></cs-toast>`);

          const container = document.querySelector('[data-cs-toast-live-region]');
          expect(container).to.exist;
          expect(container?.id).to.match(/^cs-toast-live-region-/);
        });
        ...
        it('should add a polite announcer for non-danger toasts and inject text on a later frame', async () => {
          ...
          const announcer = document.querySelector('[data-cs-toast-live-region] [aria-live="polite"]');
        ...
        it('should add an assertive announcer for danger toasts', async () => {
          ...
          const announcer = document.querySelector('[data-cs-toast-live-region] [aria-live="assertive"]');
          expect(announcer).to.exist;
        ...
        it('should add a separate announcer node for each toast', async () => {
          ...
          const announcers = document.querySelectorAll('[data-cs-toast-live-region] > div');
          expect(announcers.length).to.equal(3);
        });

        it('should clean up the live region container when removed from the DOM', async () => {
          const el = await fixture<CsToast>(html`<cs-toast></cs-toast>`);
          expect(document.querySelector('[data-cs-toast-live-region]')).to.exist;

          el.remove();
          await aTimeout(0);

          expect(document.querySelector('[data-cs-toast-live-region]')).to.not.exist;
        });
      });

      describe('toast item lifecycle', () => {
        it('should remove toast item from DOM after cs-after-hide', async () => {
          const el = await fixture<CsToast>(html`<cs-toast></cs-toast>`);
          const toastItem = await el.create('Test message', { duration: 0 });

          expect(el.querySelectorAll('cs-toast-item').length).to.equal(1);

          toastItem.hide();

          await waitUntil(() => el.querySelectorAll('cs-toast-item').length === 0, 'Toast item should be removed', {
            timeout: 1000,
          });

          expect(el.querySelectorAll('cs-toast-item').length).to.equal(0);
        });
        ...
        it('should keep stack visible when there are remaining toast items', async () => {
          ...
          await waitUntil(() => el.querySelectorAll('cs-toast-item').length === 1, 'One toast item should remain', {
            timeout: 1000,
          });

          // `length === 1` is observable mid-teardown, before the stack has reacted to the removal, so
          // asserting straight after it races the component. Let the update settle first — the invariant
          // under test is the settled state, not the instant the count changes.
          await el.updateComplete;
          await nextFrame();
      ...
      describe('manual toast items via slot', () => {
        it('should activate slotted toast items', async () => {
          const el = await fixture<CsToast>(html`
            <cs-toast>
              <cs-toast-item>Slotted message</cs-toast-item>
            </cs-toast>
          `);
      ...
      describe('keyboard interaction', () => {
        it('should dismiss the oldest toast item when Escape is pressed', async () => {
          ...
          expect(el.querySelectorAll('cs-toast-item').length).to.equal(3);
          ...
          await waitUntil(
            () => el.querySelectorAll('cs-toast-item').length === 2,
            'One toast item should be dismissed',
            { timeout: 1000 },
          );

          expect(el.querySelectorAll('cs-toast-item').length).to.equal(2);
          // The oldest (last in DOM order) should be dismissed, leaving the two most recent
          const remainingItems = el.querySelectorAll('cs-toast-item');
      ...
      describe('multiple toast instances', () => {
        it('should work with multiple toast containers on the page', async () => {
          const container = await fixture<HTMLDivElement>(html`
            <div>
              <cs-toast id="toast1" placement="top-start"></cs-toast>
              <cs-toast id="toast2" placement="bottom-end"></cs-toast>
            </div>
          `);

          const toast1 = container.querySelector<CsToast>('#toast1')!;
          const toast2 = container.querySelector<CsToast>('#toast2')!;

          await toast1.create('Message in toast 1');
          await toast2.create('Message in toast 2');

          expect(toast1.querySelectorAll('cs-toast-item').length).to.equal(1);
          expect(toast2.querySelectorAll('cs-toast-item').length).to.equal(1);
        });
      });

      describe('events from toast items', () => {
        it('should bubble cs-show event from toast item', async () => {
          const el = await fixture<CsToast>(html`<cs-toast></cs-toast>`);
        ...
        it('should bubble cs-after-show event from toast item', async () => {
          const el = await fixture<CsToast>(html`<cs-toast></cs-toast>`);
        ...
        it('should bubble cs-hide event from toast item', async () => {
          const el = await fixture<CsToast>(html`<cs-toast></cs-toast>`);
        ...
        it('should bubble cs-after-hide event from toast item', async () => {
          const el = await fixture<CsToast>(html`<cs-toast></cs-toast>`);
      ...
      describe('auto-dismiss behavior', () => {
        it('should auto-dismiss toast item after duration', async () => {
          ...
          await waitUntil(() => el.querySelectorAll('cs-toast-item').length === 0, 'Toast should auto-dismiss', {
            timeout: 1000,
          });
