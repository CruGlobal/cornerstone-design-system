...
export default class CsOption extends CornerstoneElement {
  static css = styles;
  ...
  private handleDefaultSlotChange() {
    ...
    if (this.isInitialized) {
      // When the label changes, tell the parent <cs-select> to update. The parent's handleDefaultSlotChange already
      ...
      // When the label changes, tell the parent <cs-combobox> to update
      customElements.whenDefined('cs-combobox').then(() => {
        // We cast to <cs-select> because it shares the same API as combobox
        const controller = this.closest<CsSelect>('cs-combobox');
        if (controller) {
          controller.handleDefaultSlotChange?.();
        }
      });
  ...
  syncDefaultSelected() {
    ...
    if ('closest' in this) {
      // SSR guard.
      if (!this.closest<CsSelect>('cs-combobox, cs-select')?.hasInteracted) {
  ...
  protected async firstUpdated(changedProperties: PropertyValues<this>) {
    ...
    if (this.selected && !this.defaultSelected) {
      const parent = this.closest<CsSelect>('cs-select, cs-combobox');
  ...
  render() {
    ...
    return html`
      ${
        selected
          ? html`<cs-icon part="checked-icon" class="check" name="check" library="system" aria-hidden="true"></cs-icon>`
