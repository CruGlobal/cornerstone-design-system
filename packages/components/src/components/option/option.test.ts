...
import type CsSelect from '../select/select.js';
import type CsOption from './option.js';
    ...
    describe(`with "${fixture.type}" rendering`, () => {
      describe('accessibility', () => {
        it('should pass accessibility tests when used inside a select', async () => {
          const el = await fixture<CsSelect>(html`
            <cs-select label="Select one">
              ...
              <cs-option value="3">Option 3</cs-option>
              <cs-option value="4" disabled>Disabled</cs-option>
            </cs-select>
          `);
        ...
        it('should have role="option"', async () => {
          const el = await fixture<CsOption>(html` <cs-option>Test</cs-option> `);
        ...
        it('should set aria-disabled when disabled', async () => {
          const el = await fixture<CsOption>(html` <cs-option disabled>Test</cs-option> `);
      ...
      describe('properties', () => {
        ...
        it('should reflect the value property to an attribute', async () => {
          const el = await fixture<CsOption>(html` <cs-option value="test">Test</cs-option> `);
          expect(el.getAttribute('value')).to.equal('test');
        });

        it('should convert non-string values to string', async () => {
          const el = await fixture<CsOption>(html` <cs-option>Text</cs-option> `);
        ...
        it('should generate defaultLabel from text content', async () => {
          const el = await fixture<CsOption>(html` <cs-option><strong>Option</strong></cs-option> `);
        ...
        it('should allow the label attribute to override defaultLabel', async () => {
          const el = await fixture<CsOption>(html` <cs-option label="Manual label">Text content</cs-option> `);
        ...
        it('should fall back to defaultLabel when label is empty', async () => {
          const el = await fixture<CsOption>(html` <cs-option>Fallback</cs-option> `);
      ...
      describe('CSS parts and states', () => {
        ...
        it('should render a cs-icon for checked-icon when selected', async () => {
          const el = await fixture<CsOption>(html` <cs-option>Test</cs-option> `);
          el.selected = true;
          await el.updateComplete;
          const icon = el.shadowRoot!.querySelector('cs-icon[part~="checked-icon"]');
      ...
      describe('within a select', () => {
        it('should be selectable inside a cs-select', async () => {
          const select = await fixture<CsSelect>(html`
            <cs-select label="Test" value="2">
              <cs-option value="1">One</cs-option>
              <cs-option value="2">Two</cs-option>
              <cs-option value="3">Three</cs-option>
            </cs-select>
          `);
          await aTimeout(0);

          const options = select.querySelectorAll<CsOption>('cs-option');
          expect(options[1].selected).to.be.true;
          expect(options[0].selected).to.be.false;
        });

        it('should support the disabled state inside a cs-select', async () => {
          const select = await fixture<CsSelect>(html`
            <cs-select label="Test">
              <cs-option value="1">One</cs-option>
              <cs-option value="2" disabled>Two</cs-option>
            </cs-select>
          `);

          const disabledOption = select.querySelector<CsOption>('cs-option[disabled]')!;
