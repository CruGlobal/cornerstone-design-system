...
import { CsSelectEvent } from '../../events/select.js';
...
const openDropdowns = new Set<CsDropdown>();
...
export default class CsDropdown extends CornerstoneElement {
  static css = [sizeStyles, styles];

  private submenuCleanups: Map<CsDropdownItem, ReturnType<typeof autoUpdate>> = new Map();
  private readonly localize = new LocalizeController(this);
  private userTypedQuery = '';
  private userTypedTimeout: ReturnType<typeof setTimeout>;
  private openSubmenuStack: CsDropdownItem[] = [];

  @query('slot:not([name])') defaultSlot: HTMLSlotElement;
  @query('#menu') private menu: HTMLDivElement;
  @query('cs-popup') private popup: CsPopup;
  ...
  private getItems(includeDisabled = false): CsDropdownItem[] {
    const items = (this.defaultSlot?.assignedElements({ flatten: true }) ?? []).filter(
      (el) => el.localName === 'cs-dropdown-item',
    ) as CsDropdownItem[];
  ...
  private getSubmenuItems(parentItem: CsDropdownItem, includeDisabled = false): CsDropdownItem[] {
    ...
    const items = submenuSlot
      ...
      .filter((el) => el.localName === 'cs-dropdown-item') as CsDropdownItem[];
  ...
  private addToSubmenuStack(item: CsDropdownItem) {
  ...
  private getCurrentSubmenuItem(): CsDropdownItem | undefined {
  ...
  private closeSiblingSubmenus(item: CsDropdownItem) {
    const parentDropdownItem = item.closest<CsDropdownItem>('cs-dropdown-item:not([slot="submenu"])');
    let siblingItems: CsDropdownItem[];
  ...
  /** Get the slotted trigger button, a <cs-button> or <button> element */
  private getTrigger(): HTMLButtonElement | CsButton | null {
    return this.querySelector<CsButton | HTMLButtonElement>('[slot="trigger"]');
  }

  /** Shows the dropdown menu. This should only be called from within updated(). */
  private async showMenu() {
    const anchor = this.getTrigger();
    if (!anchor || !this.popup || !this.menu) {
      return;
    }
    ...
    // (this can happen when cs-hide was cancelled)
    ...
    this.popup.active = true; // Use cs-popup's active property instead of showPopover
    this.open = true;
  ...
  private async hideMenu() {
    ...
    const hideEvent = new CsHideEvent({ source: this });
    ...
    this.popup.active = this.open; // Hide using cs-popup
  ...
  private handleDocumentKeyDown = async (event: KeyboardEvent) => {
    ...
    const activeElement = [...activeElements()].find((el) => el.localName === 'cs-dropdown-item');
    const isFocusedOnItem = activeElement?.localName === 'cs-dropdown-item';
    const currentSubmenuItem = this.getCurrentSubmenuItem();
    const isInSubmenu = !!currentSubmenuItem;

    let items: CsDropdownItem[];
    let activeItem: CsDropdownItem | undefined;
    ...
    let itemToSelect: CsDropdownItem | undefined;
        ...
        setTimeout(() => {
          ...
          const parentItems =
            removedItem.slot === 'submenu'
              ? this.getSubmenuItems(removedItem.parentElement as CsDropdownItem)
              : this.getItems();
  ...
  private handleDocumentPointerDown = (event: PointerEvent) => {
    ...
    const isInDropdownHierarchy = path.some((el) => {
      if (el instanceof HTMLElement) {
        return el === this || el.closest('cs-dropdown, [part="submenu"]');
      }
      return false;
    });
  ...
  private handleMenuClick(event: MouseEvent) {
    const item = (event.target as Element).closest('cs-dropdown-item');
  ...
  private handleSubmenuOpening(event: CustomEvent) {
    const openingItem = event.detail.item as CsDropdownItem;
  ...
  private setupSubmenuPosition(item: CsDropdownItem) {
    ...
    if (submenuSlot) {
      submenuSlot.removeEventListener('slotchange', CsDropdown.handleSubmenuSlotChange);
      submenuSlot.addEventListener('slotchange', CsDropdown.handleSubmenuSlotChange);
      CsDropdown.handleSubmenuSlotChange({ target: submenuSlot } as unknown as Event);
    }
  }
  ...
  private static handleSubmenuSlotChange(event: Event) {
    ...
    const items = slot.assignedElements().filter((el) => el.localName === 'cs-dropdown-item') as CsDropdownItem[];
  ...
  private processSubmenuItems(item: CsDropdownItem) {
    if (!item.submenuElement) {
      return;
    }
  ...
  private cleanupSubmenuPosition(item: CsDropdownItem) {
  ...
  private positionSubmenu(item: CsDropdownItem) {
    if (!item.submenuElement) {
      return;
    }
  ...
  private updateSafeTriangleCoordinates(item: CsDropdownItem) {
  ...
  private handleGlobalMouseMove = (event: MouseEvent) => {
    ...
    if (!isOverItem && !isOverSubmenu) {
      setTimeout(() => {
        // Re-read hover state: the delay is what lets the pointer cross the gap into the submenu.
        const stillOverItem = currentSubmenuItem.matches(':hover');
        const stillOverSubmenu = Boolean(currentSubmenuItem.submenuElement?.matches(':hover'));

        if (!stillOverItem && !stillOverSubmenu) {
  ...
  /** Makes a selection, emits the cs-select event, and closes the dropdown. */
  private makeSelection(item: CsDropdownItem) {
    const trigger = this.getTrigger();
    ...
    const selectEvent = new CsSelectEvent({ item });
  ...
  private async syncAriaAttributes() {
    ...
    if (trigger.localName === 'cs-button') {
      await customElements.whenDefined('cs-button');
      await (trigger as CsButton).updateComplete;
    ...
    if (!nativeButton.hasAttribute('id')) {
      nativeButton.setAttribute('id', uniqueId('cs-dropdown-trigger-'));
    }
  ...
  render() {
    ...
    const active = this.didSSR && !this.hasUpdated ? this.open : this.popup?.active;
