...
import type CsAccordionItem from './accordion-item.js';

describe('<cs-accordion-item>', () => {
  for (const fixture of fixtures) {
    describe(`with "${fixture.type}" rendering`, () => {
      describe('accessibility', () => {
        it('should be accessible when collapsed', async () => {
          const el = await fixture<CsAccordionItem>(html`<cs-accordion-item label="Test">Content</cs-accordion-item>`);
          await expect(el).to.be.accessible();
        });

        it('should be accessible when expanded', async () => {
          const el = await fixture<CsAccordionItem>(
            html`<cs-accordion-item label="Test" expanded>Content</cs-accordion-item>`,
          );
          await expect(el).to.be.accessible();
        });
        ...
        it('should set aria-disabled when disabled', async () => {
          const el = await fixture<CsAccordionItem>(
            html`<cs-accordion-item label="Test" disabled>Content</cs-accordion-item>`,
          );
          const button = el.shadowRoot!.querySelector('[part~="button"]')!;
      ...
      describe('properties', () => {
        ...
        it('should default label to empty string', async () => {
          const el = await fixture<CsAccordionItem>(html`<cs-accordion-item>Content</cs-accordion-item>`);
        ...
        it('should use the label attribute as the header text', async () => {
          const el = await fixture<CsAccordionItem>(
            html`<cs-accordion-item label="My Label">Content</cs-accordion-item>`,
          );
      ...
      describe('methods', () => {
        ...
        it('should not collapse when calling collapse() on a disabled item', async () => {
          const el = await fixture<CsAccordionItem>(
            html`<cs-accordion-item label="Test" expanded disabled>Content</cs-accordion-item>`,
          );
          await el.collapse();
          expect(el.expanded).to.be.true;
        });
      ...
      describe('slots', () => {
        it('should render the default slot content', async () => {
          const el = await fixture<CsAccordionItem>(html`
            <cs-accordion-item label="Test" expanded><span id="content">Hello</span></cs-accordion-item>
          `);
      ...
      describe('CSS parts', () => {
        it('should expose the base part', async () => {
          const el = await fixture<CsAccordionItem>(html`<cs-accordion-item label="Test">Content</cs-accordion-item>`);
          expect(el.shadowRoot!.querySelector('[part~="accordion-item"]')).to.exist;
        });
      ...
      describe('rapid toggling', () => {
        ...
        it('should only fire the final after event when the animation is interrupted', async () => {
          ...
          el.addEventListener('cs-accordion-item-expanded', () => expandedCount++);
          el.addEventListener('cs-accordion-item-collapsed', () => collapsedCount++);
      ...
      describe('trigger event', () => {
        it('should dispatch the internal trigger event when the button is clicked', async () => {
          ...
          await expectEvent(el, 'cs-accordion-item-trigger', () => button.click());
        });
        ...
        it('should not dispatch the trigger event when disabled', async () => {
          ...
          const button = el.shadowRoot!.querySelector<HTMLButtonElement>('[part~="button"]')!;
          let fired = false;
          el.addEventListener('cs-accordion-item-trigger', () => (fired = true));
          button.click();
          await el.updateComplete;
