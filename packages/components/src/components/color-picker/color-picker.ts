...
import { CsInvalidEvent } from '../../events/invalid.js';
...
import type CsInput from '../input/input.js';
 ...
 * @csspart eyedropper-button__button - The eye dropper button's exported `button` part.
 ...
 */
@customElement('cs-color-picker')
export default class CsColorPicker extends CornerstoneFormAssociatedElement {
  ...
  @query('[part~="color-picker"]') base: HTMLElement;
  @query('[part~="input"]') input: CsInput;
  ...
  @query('.color-popup') popup: CsPopup;
   ...
   */
  @property({ attribute: 'ssr-label', reflect: true, type: Boolean }) ssrLabel = false;

  /**
   * Only required for SSR. Set to `true` if you're slotting in a `hint` element so the server-rendered markup
   * includes the hint before the component hydrates on the client.
   */
  @property({ attribute: 'ssr-hint', reflect: true, type: Boolean }) ssrHint = false;
  ...
  private handleInputChange(event: Event) {
    ...
    // Prevent the `<cs-input>` element's `change` event from bubbling up
  ...
  private handleInputInput(event: InputEvent) {
    ...
    // Prevent the `<cs-input>` element's `input` event from bubbling up
    event.stopPropagation();
  }
  ...
  private reportValidityAfterShow = () => {
    // Remove the event so we don't emit "cs-invalid" twice
  ...
  reportValidity() {
    ...
    if (!this.validity.valid && !this.open) {
      ...
      this.addEventListener('cs-after-show', this.reportValidityAfterShow, { once: true });
      this.show();

      if (!this.disabled) {
        // By standards we have to emit a `cs-invalid` event here synchronously.
  ...
  async handleOpenChange() {
    ...
    if (this.open) {
      // Show
      this.dispatchEvent(new CustomEvent('cs-show'));
      ...
      this.dispatchEvent(new CustomEvent('cs-after-show'));
    } else {
      // Hide
      this.dispatchEvent(new CustomEvent('cs-hide'));

      this.removeOpenListeners();
      await animateWithClass(this.popup.popup, 'hide-with-scale');
      this.base.hidden = true;
      this.popup.active = false;
      this.dispatchEvent(new CustomEvent('cs-after-hide'));
    }
  }
  ...
  render() {
              ...
              this.hasEyeDropper
                ? html`
                    <cs-button
                      ...
                      appearance="outlined"
                      exportparts="
                      button:eyedropper-button__button,
    ...
    return html`
      ...
      <cs-popup
        ...
        shift-padding="10"
        aria-disabled=${this.disabled ? 'true' : 'false'}
        @cs-after-show=${this.handleAfterShow}
        @cs-after-hide=${this.handleAfterHide}
      >
        ${colorPicker}
      </cs-popup>
    `;
...
//
// - The base class (CornerstoneFormAssociatedElement) firstUpdated() calls updateValidity() which triggers
