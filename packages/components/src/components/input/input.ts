...
export default class CsInput extends CornerstoneFormAssociatedElement {
  ...
  formResetCallback() {
    ...
    if (this.input) {
      // Fixes https://github.com/shoelace-style/webawesome/issues/1640 where resetting an input would leave the "live" value in place on the input in the shadow dom. This fixed that by manually forcing the value.
      // @ts-expect-error -- `value` is a getter on the base class; writing through to the inner input is deliberate
