...
import type CsFormatBytes from './format-bytes.js';

describe('<cs-format-bytes>', () => {
  for (const fixture of fixtures) {
    describe(`with "${fixture.type}" rendering`, () => {
      describe('properties', () => {
        it('should have correct default values', async () => {
          const el = await fixture<CsFormatBytes>(html`<cs-format-bytes></cs-format-bytes>`);
      ...
      describe('byte formatting', () => {
        ...
        byteCases.forEach(({ value, short, long, narrow }) => {
          it(`should format ${value} bytes correctly in short display`, async () => {
            const el = await fixture<CsFormatBytes>(html`<cs-format-bytes value="${value}"></cs-format-bytes>`);
            expect(el.shadowRoot?.textContent).to.equal(short);
          });

          it(`should format ${value} bytes correctly in long display`, async () => {
            const el = await fixture<CsFormatBytes>(
              html`<cs-format-bytes value="${value}" display="long"></cs-format-bytes>`,
            );
          ...
          it(`should format ${value} bytes correctly in narrow display`, async () => {
            const el = await fixture<CsFormatBytes>(
              html`<cs-format-bytes value="${value}" display="narrow"></cs-format-bytes>`,
            );
      ...
      describe('bit formatting', () => {
        ...
        bitCases.forEach(({ value, short, long, narrow }) => {
          it(`should format ${value} bits correctly in short display`, async () => {
            const el = await fixture<CsFormatBytes>(
              html`<cs-format-bytes value="${value}" unit="bit"></cs-format-bytes>`,
            );
            expect(el.shadowRoot?.textContent).to.equal(short);
          });
          ...
          it(`should format ${value} bits correctly in long display`, async () => {
            const el = await fixture<CsFormatBytes>(
              html`<cs-format-bytes value="${value}" unit="bit" display="long"></cs-format-bytes>`,
            );
            expect(el.shadowRoot?.textContent).to.equal(long);
          });
          ...
          it(`should format ${value} bits correctly in narrow display`, async () => {
            const el = await fixture<CsFormatBytes>(
              html`<cs-format-bytes value="${value}" unit="bit" display="narrow"></cs-format-bytes>`,
            );
            expect(el.shadowRoot?.textContent).to.equal(narrow);
          });
        });
      ...
      describe('edge cases', () => {
        ...
        it('should update when properties change dynamically', async () => {
          const el = await fixture<CsFormatBytes>(html`<cs-format-bytes value="1200"></cs-format-bytes>`);
