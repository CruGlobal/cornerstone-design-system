...
import type CsDropdownItem from './dropdown-item.js';

describe('<cs-dropdown-item>', () => {
  for (const fixture of fixtures) {
    describe(`with "${fixture.type}" rendering`, () => {
      describe('accessibility', () => {
        it('should pass accessibility tests', async () => {
          // `role="menuitem"` requires a menu ancestor. Checked inside a role-correct container rather
          // than standalone, which reports a false `aria-required-parent` — and rather than inside a real
          // `<cs-dropdown>`, which would make this an integration test of the parent's semantics instead.
          const el = await fixture(html`<div role="menu"><cs-dropdown-item>Item</cs-dropdown-item></div>`);
      ...
      describe('accessibility', () => {
        it('should have role="menuitem" by default', async () => {
          const el = await fixture<CsDropdownItem>(html`<cs-dropdown-item>Item</cs-dropdown-item>`);
          expect(el.getAttribute('role')).to.equal('menuitem');
        });

        it('should have role="menuitemcheckbox" when type is checkbox', async () => {
          const el = await fixture<CsDropdownItem>(html`<cs-dropdown-item type="checkbox">Item</cs-dropdown-item>`);
        ...
        it('should have aria-checked="true" when type is checkbox and checked', async () => {
          const el = await fixture<CsDropdownItem>(
            html`<cs-dropdown-item type="checkbox" checked>Item</cs-dropdown-item>`,
          );
        ...
        it('should set aria-disabled when disabled', async () => {
          const el = await fixture<CsDropdownItem>(html`<cs-dropdown-item disabled>Item</cs-dropdown-item>`);
      ...
      describe('properties', () => {
        ...
        it('should reflect the variant property to an attribute', async () => {
          const el = await fixture<CsDropdownItem>(html`<cs-dropdown-item variant="danger">Item</cs-dropdown-item>`);
      ...
      describe('slots', () => {
        it('should render slotted content in the default slot', async () => {
          const el = await fixture<CsDropdownItem>(html`<cs-dropdown-item>My Item</cs-dropdown-item>`);
        ...
        it('should accept content in the icon slot', async () => {
          const el = await fixture<CsDropdownItem>(html`
            <cs-dropdown-item>
