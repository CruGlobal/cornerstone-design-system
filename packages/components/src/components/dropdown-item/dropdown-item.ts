...
export default class CsDropdownItem extends CornerstoneElement {
  static css = styles;
  ...
  @property({ reflect: true }) variant: 'danger' | 'neutral' = 'neutral';
  ...
  private notifyParentOfOpening() {
    ...
    if (parent) {
      const siblings = [...parent.children].filter(
        (el) =>
          el !== this &&
          el.localName === 'cs-dropdown-item' &&
          el.getAttribute('slot') === this.getAttribute('slot') &&
          (el as CsDropdownItem).submenuOpen,
      ) as CsDropdownItem[];
  ...
  private getSubmenuItems(): CsDropdownItem[] {
    // Only get direct children with slot="submenu", not nested ones
    return [...this.children].filter(
      (el) =>
        el.localName === 'cs-dropdown-item' && el.getAttribute('slot') === 'submenu' && !el.hasAttribute('disabled'),
    ) as CsDropdownItem[];
  }
...
declare global {
  interface HTMLElementTagNameMap {
    'cs-dropdown-item': CsDropdownItem;
  }
}
