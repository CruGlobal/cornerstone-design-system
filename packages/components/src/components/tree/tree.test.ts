...
import type CsTreeItem from '../tree-item/tree-item.js';
import type CsTree from './tree.js';
    ...
    describe(`with "${fixture.type}" rendering`, () => {
      ...
      beforeEach(async () => {
        el = await fixture(html`
          <cs-tree>
            ...
            <cs-tree-item>Node 2</cs-tree-item>
            <cs-tree-item id="expandable">
              Parent Node
              <cs-tree-item>Child Node 1</cs-tree-item>
              <cs-tree-item>
                Child Node 2
                <cs-tree-item>Child Node 2 - 1</cs-tree-item>
                <cs-tree-item>Child Node 2 - 2</cs-tree-item>
              </cs-tree-item>
            </cs-tree-item>
            <cs-tree-item>Node 3</cs-tree-item>
          </cs-tree>
        `);
      });
      ...
      describe('properties', () => {
        ...
        it('should set selectable on leaf items only when selection is "leaf-multiple"', async () => {
          ...
          const items = el.querySelectorAll<CsTreeItem>('cs-tree-item');
          await Promise.all([...items].map((item) => item.updateComplete));
        ...
        it('should not focus collapsed child nodes', async () => {
          const parentNode = el.children[2] as CsTreeItem;
          const childNode = parentNode.children[1] as CsTreeItem;
        ...
        it('should return selected items', async () => {
          el.selection = 'single';
          const node = el.children[0] as CsTreeItem;
          node.focus();
          await el.updateComplete;
      ...
      describe('slots', () => {
        ...
        describe('when a custom expanded/collapsed icon is provided', () => {
          ...
          it('should append a clone of the icon in the proper slot of the tree item', async () => {
            ...
            const treeItems = [...el.querySelectorAll('cs-tree-item')];
        ...
        it("should not render 'null' if it can't find a custom icon", async () => {
          const tree = await fixture<CsTree>(html`
            <cs-tree>
              <cs-tree-item>
                Item 1
                <cs-icon name="looks_one" slot="expand-icon"></cs-icon>
                <cs-tree-item> Item A </cs-tree-item>
              </cs-tree-item>
              <cs-tree-item>
                Item 2
                <cs-tree-item>Item A</cs-tree-item>
                <cs-tree-item>Item B</cs-tree-item>
              </cs-tree-item>
              <cs-tree-item>
                Item 3
                <cs-tree-item>Item A</cs-tree-item>
      ...
      describe('keyboard navigation', () => {
        ...
        describe('when Home is pressed', () => {
          it('should move the focus to the first tree item', async () => {
            const parentNode = el.children[3] as CsTreeItem;
            parentNode.focus();
            await el.updateComplete;
        ...
        describe('when End is pressed', () => {
          it('should move the focus to the last tree item', async () => {
            const parentNode = el.children[0] as CsTreeItem;
            parentNode.focus();
            await el.updateComplete;
        ...
        describe('when Enter is pressed', () => {
          describe('and selection is "single"', () => {
            it('should select only one tree item', async () => {
              el.selection = 'single';
              const node = el.children[1] as CsTreeItem;
              node.focus();
              await el.updateComplete;
        ...
        describe('when focus is inside a tree item instead of on it', () => {
          ...
          async function fixtureWithLink() {
            const tree = await fixture<CsTree>(html`
              <cs-tree>
                <cs-tree-item><a href="#node-1">Node 1</a></cs-tree-item>
                <cs-tree-item>Node 2</cs-tree-item>
              </cs-tree>
            `);
      ...
      describe('events', () => {
        it('should emit cs-selection-change when selecting via keyboard', async () => {
          el.selection = 'single';
          const node = el.children[0] as CsTreeItem;
          node.focus();
          await el.updateComplete;

          const events = await expectEvent(el, 'cs-selection-change', async () => {
            await sendKeys({ press: 'Enter' });
          });
        ...
        describe('when selection is "leaf"', () => {
          it('should not emit cs-selection-change when clicking an expandable item', async () => {
            el.selection = 'leaf';
            await el.updateComplete;

            const selectedChangeSpy = sinon.spy();
            el.addEventListener('cs-selection-change', selectedChangeSpy);

            const node = el.querySelector<CsTreeItem>('#expandable')!;
        ...
        describe('when selection is "leaf-multiple"', () => {
          ...
          it('should emit cs-selection-change when clicking a leaf item', async () => {
            el.selection = 'leaf-multiple';
            await el.updateComplete;
      ...
      describe('checkbox synchronization', () => {
        it('should select all nested children when a parent node is selected', async () => {
          const tree = await fixture<CsTree>(html`
            <cs-tree selection="multiple">
              <cs-tree-item selected>
                Parent Node
                <cs-tree-item selected>Child Node 1</cs-tree-item>
                <cs-tree-item>
                  Child Node 2
                  <cs-tree-item>Child Node 2 - 1</cs-tree-item>
                  <cs-tree-item>Child Node 2 - 2</cs-tree-item>
          ...
          `);
          const treeItems = Array.from<CsTreeItem>(tree.querySelectorAll('cs-tree-item'));
