...
import { CsTabHideEvent } from '../../events/tab-hide.js';
import { CsTabShowEvent } from '../../events/tab-show.js';
...
export default class CsTabGroup extends CornerstoneElement {
  static css = styles;

  private activeTab?: CsTab;
  private mutationObserver: MutationObserver;
  private resizeObserver: ResizeObserver;
  private tabs: CsTab[] = [];
  private focusableTabs: CsTab[] = [];
  private panels: CsTabPanel[] = [];
    ...
    this.mutationObserver = new MutationObserver((mutations) => {
      ...
      if (relevantMutations.some((m) => m.attributeName === 'disabled')) {
        ...
        const tabs = relevantMutations
          .filter((m) => m.attributeName === 'active' && (m.target as HTMLElement).tagName.toLowerCase() === 'cs-tab')
          .map((m) => m.target as CsTab);
  ...
  private getAllTabs() {
    ...
    return [...(slot.assignedElements() as CsTab[])].filter((el) => {
      return el.tagName.toLowerCase() === 'cs-tab';
    });
  }

  private getAllPanels() {
    return [...this.defaultSlot.assignedElements()].filter((el) => el.tagName.toLowerCase() === 'cs-tab-panel') as [
      CsTabPanel,
    ];
  }
  ...
  private setActiveTab(tab: CsTab, options?: { emitEvents?: boolean; scrollBehavior?: 'auto' | 'smooth' }) {
    options = {
      emitEvents: true,
      scrollBehavior: 'auto',
      ...options,
