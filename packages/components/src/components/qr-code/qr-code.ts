 ...
 */
@customElement('cs-qr-code')
export default class CsQrCode extends CornerstoneElement {
  static css = styles;
  ...
  generate() {
    ...
    if (span) {
      // @ts-expect-error -- spanComputedStyle is assigned lazily and typed as required
    ...
    QrCreator.render(
      {
        ...
        // @ts-expect-error -- cornerFill is not in the QrCreator option types, but the library reads it
