...
import type CsTimeInput from './time-input.js';
...
function getSegments(el: CsTimeInput): HTMLElement[] {
  return Array.from(el.shadowRoot!.querySelectorAll<HTMLElement>('[data-segment]'));
}

function getSegment(el: CsTimeInput, field: 'hour' | 'minute' | 'second' | 'dayPeriod'): HTMLElement {
...
describe('<cs-time-input>', () => {
  runFormControlBaseTests({ tagName: 'cs-time-input', formValue: { dirtyValue: '10:30' } });

  describe('accessibility', () => {
    it('is accessible with a label and value', async () => {
      const el = await fixture<CsTimeInput>(
        html`<cs-time-input label="Start" value="09:30" lang="en-GB"></cs-time-input>`,
      );
  ...
  describe('basic rendering', () => {
    it('renders with default attributes', async () => {
      const el = await fixture<CsTimeInput>(html`<cs-time-input></cs-time-input>`);
      expect(el).to.exist;
    ...
    it('hydrates value from the attribute', async () => {
      const el = await fixture<CsTimeInput>(html`<cs-time-input value="14:30" lang="en-GB"></cs-time-input>`);
      await el.updateComplete;
    ...
    it('exposes valueAsNumber (ms since midnight)', async () => {
      const el = await fixture<CsTimeInput>(html`<cs-time-input value="01:00" lang="en-GB"></cs-time-input>`);
      await el.updateComplete;
      expect(el.valueAsNumber).to.equal(3_600_000);
    });

    it('reflects the initial value in valueAsDate before any interaction', async () => {
      const el = await fixture<CsTimeInput>(html`<cs-time-input value="09:30" lang="en-GB"></cs-time-input>`);
      await el.updateComplete;
  ...
  describe('segmented input structure', () => {
    it('renders 2 segments by default (hour, minute) in 24-hour locale', async () => {
      const el = await fixture<CsTimeInput>(html`<cs-time-input lang="en-GB"></cs-time-input>`);
    ...
    it('renders 3 segments in 12-hour locale (hour, minute, dayPeriod)', async () => {
      const el = await fixture<CsTimeInput>(html`<cs-time-input lang="en-US"></cs-time-input>`);
      const segments = getSegments(el);
      const fields = segments.map((s) => s.dataset.segment);
    ...
    it('adds a seconds segment when step is sub-minute', async () => {
      const el = await fixture<CsTimeInput>(html`<cs-time-input lang="en-GB" step="1"></cs-time-input>`);
      await el.updateComplete;
    ...
    it('hides the seconds segment when step is a whole-minute multiple', async () => {
      const el = await fixture<CsTimeInput>(html`<cs-time-input lang="en-GB" step="300"></cs-time-input>`);
      await el.updateComplete;
      const fields = getSegments(el).map((s) => s.dataset.segment);
    ...
    it('forces 24-hour layout when hour-format=24', async () => {
      const el = await fixture<CsTimeInput>(html`<cs-time-input lang="en-US" hour-format="24"></cs-time-input>`);
      await el.updateComplete;
      const fields = getSegments(el).map((s) => s.dataset.segment);
    ...
    it('forces 12-hour layout when hour-format=12', async () => {
      const el = await fixture<CsTimeInput>(html`<cs-time-input lang="en-GB" hour-format="12"></cs-time-input>`);
      await el.updateComplete;
      const fields = getSegments(el).map((s) => s.dataset.segment);
  ...
  describe('AM/PM segment', () => {
    it('toggles dayPeriod via "p" key', async () => {
      const el = await fixture<CsTimeInput>(html`<cs-time-input lang="en-US" value="09:00"></cs-time-input>`);
      await el.updateComplete;
    ...
    it('toggles dayPeriod via "a" key', async () => {
      const el = await fixture<CsTimeInput>(html`<cs-time-input lang="en-US" value="21:00"></cs-time-input>`);
      await el.updateComplete;
      const dp = getSegment(el, 'dayPeriod');
      dp.focus();
      type(dp, 'a');
  ...
  describe('Arrow stepping', () => {
    it('hour wraps within bounds with no carry into minutes', async () => {
      const el = await fixture<CsTimeInput>(html`<cs-time-input lang="en-GB" value="23:30"></cs-time-input>`);
      await el.updateComplete;
      const hour = getSegment(el, 'hour');
      hour.focus();
      type(hour, 'ArrowUp');
    ...
    it('minute wraps without carrying into hour', async () => {
      const el = await fixture<CsTimeInput>(html`<cs-time-input lang="en-GB" value="10:59"></cs-time-input>`);
      await el.updateComplete;
      const minute = getSegment(el, 'minute');
      minute.focus();
      type(minute, 'ArrowUp');
    ...
    it('hour 12 → 1 in 12-hour mode without toggling AM/PM', async () => {
      const el = await fixture<CsTimeInput>(html`<cs-time-input lang="en-US" value="12:00"></cs-time-input>`);
      await el.updateComplete;
  ...
  describe('clear button', () => {
    it('clears the value and refocuses the first segment', async () => {
      const el = await fixture<CsTimeInput>(
        html`<cs-time-input lang="en-GB" value="10:00" with-clear></cs-time-input>`,
      );
      await el.updateComplete;
  ...
  describe('popup', () => {
    it('opens when the expand button is clicked', async () => {
      ...
      expandButton.click();
      await waitForEvent(el, 'cs-after-show');
    ...
    it('closes via Alt+ArrowUp', async () => {
      ...
      await waitForEvent(el, 'cs-after-hide');
      expect(el.open).to.equal(false);
    });
  ...
  describe('Now button', () => {
    it('renders only when with-now is set', async () => {
      const noNow = await fixture<CsTimeInput>(html`<cs-time-input lang="en-GB"></cs-time-input>`);
      noNow.show();
      await waitForEvent(noNow, 'cs-after-show');
      expect(noNow.shadowRoot!.querySelector('.now-button')).to.equal(null);

      const withNow = await fixture<CsTimeInput>(html`<cs-time-input lang="en-GB" with-now></cs-time-input>`);
      withNow.show();
      await waitForEvent(withNow, 'cs-after-show');
    ...
    it('clicking Now sets the value to the current time', async () => {
      const el = await fixture<CsTimeInput>(html`<cs-time-input lang="en-GB" with-now></cs-time-input>`);
      el.show();
      await waitForEvent(el, 'cs-after-show');
  ...
  describe('events', () => {
    ...
    it('emits cs-clear when the clear button is activated', async () => {
      const el = await fixture<CsTimeInput>(
        html`<cs-time-input lang="en-GB" value="10:00" with-clear></cs-time-input>`,
      );
      await el.updateComplete;
      let cleared = false;
      el.addEventListener('cs-clear', () => (cleared = true));
  ...
  describe('form association', () => {
    it('submits the wire value via form data', async () => {
      const form = await fixture<HTMLFormElement>(
        html`<form>
          <cs-time-input name="t" value="14:30" lang="en-GB"></cs-time-input>
        </form>`,
      );
      await (form.querySelector('cs-time-input') as CsTimeInput).updateComplete;
    ...
    it('resets to the default value via form reset', async () => {
      ...
      const el = form.querySelector('cs-time-input') as CsTimeInput;
      await el.updateComplete;
      el.value = '09:00';
      await el.updateComplete;
      form.reset();
    ...
    it('serializes its name and value with JSON', async () => {
      const form = await fixture<HTMLFormElement>(
        html`<form><cs-time-input name="t" value="14:30" lang="en-GB"></cs-time-input></form>`,
      );
      const el = form.querySelector<CsTimeInput>('cs-time-input')!;
      await el.updateComplete;
    ...
    it('does not add a value to the form when disabled', async () => {
      const form = await fixture<HTMLFormElement>(
        html`<form><cs-time-input name="t" value="14:30" lang="en-GB" disabled></cs-time-input></form>`,
      );
      const el = form.querySelector<CsTimeInput>('cs-time-input')!;
      await el.updateComplete;
    ...
    it('is included in form data when the form attribute points to an external form', async () => {
      const container = await fixture<HTMLDivElement>(html`
        <div>
          <form id="external"><cs-button type="submit">Submit</cs-button></form>
          <cs-time-input form="external" name="t" value="14:30" lang="en-GB"></cs-time-input>
        </div>
      `);
      const form = container.querySelector('form')!;
      const el = container.querySelector<CsTimeInput>('cs-time-input')!;
      await el.updateComplete;
      expect(new FormData(form).get('t')).to.equal('14:30');
    });

    it('resets to the default value when a cs-button[type=reset] is clicked', async () => {
    ...
    it('is invalid when required and empty and form.reportValidity() is called', async () => {
      const form = await fixture<HTMLFormElement>(
        html`<form><cs-time-input name="t" lang="en-GB" required></cs-time-input></form>`,
      );
      const el = form.querySelector<CsTimeInput>('cs-time-input')!;
      await el.updateComplete;
    ...
    it('keeps validation states when the parent form has novalidate', async () => {
      const form = await fixture<HTMLFormElement>(
        html`<form novalidate><cs-time-input lang="en-GB" required></cs-time-input></form>`,
      );
      const el = form.querySelector<CsTimeInput>('cs-time-input')!;
      await el.updateComplete;
      expect(el.validity.valueMissing).to.equal(true);
    ...
    it('emits cs-invalid from form submission when invalid', async () => {
      const form = await fixture<HTMLFormElement>(html`
        <form>
          <cs-time-input name="t" lang="en-GB" required></cs-time-input>
          <cs-button type="submit">Submit</cs-button>
        </form>
      `);
      const el = form.querySelector<CsTimeInput>('cs-time-input')!;
      const submit = form.querySelector('cs-button')!;
      await el.updateComplete;

      let invalid = false;
      el.addEventListener('cs-invalid', () => (invalid = true));
      submit.click();
  ...
  describe('validation', () => {
    it('valueMissing when required and empty', async () => {
      const el = await fixture<CsTimeInput>(html`<cs-time-input lang="en-GB" required></cs-time-input>`);
      await el.updateComplete;
      expect(el.validity.valueMissing).to.equal(true);
    });

    it('valid when required and a complete value is set', async () => {
      const el = await fixture<CsTimeInput>(html`<cs-time-input lang="en-GB" required value="09:00"></cs-time-input>`);
      await el.updateComplete;
  ...
  describe('accessibility', () => {
    it('segments have role=spinbutton with aria-valuemin/max/now/text', async () => {
      const el = await fixture<CsTimeInput>(html`<cs-time-input lang="en-GB" value="09:30"></cs-time-input>`);
      await el.updateComplete;
      const hour = getSegment(el, 'hour');
    ...
    it('the group has role=group with a label', async () => {
      const el = await fixture<CsTimeInput>(html`<cs-time-input lang="en-GB" label="Start"></cs-time-input>`);
      await el.updateComplete;
  ...
  describe('setCustomValidity / checkValidity', () => {
    ...
    it('emits cs-invalid when checkValidity() fails on a custom error', async () => {
      const el = await fixture<CsTimeInput>(html`<cs-time-input lang="en-GB" value="09:30"></cs-time-input>`);
      await el.updateComplete;
      el.setCustomValidity('Nope');
      await el.updateComplete;
      const invalidSpy = sinon.spy();
      el.addEventListener('cs-invalid', invalidSpy);
  ...
  //
  // min/max/step are mirrored onto a hidden native `<input type="time">` and surfaced into Cornerstone
  ...
  describe('min / max / step constraint validation', () => {
    it('a value below min reports rangeUnderflow and is invalid', async () => {
      const el = await fixture<CsTimeInput>(
        html`<cs-time-input lang="en-GB" min="09:00" value="08:00"></cs-time-input>`,
      );
      await el.updateComplete;
    ...
    it('a value above max reports rangeOverflow and is invalid', async () => {
      const el = await fixture<CsTimeInput>(
        html`<cs-time-input lang="en-GB" max="17:00" value="18:00"></cs-time-input>`,
      );
      await el.updateComplete;
    ...
    it('a value within [min, max] is valid', async () => {
      const el = await fixture<CsTimeInput>(
        html`<cs-time-input lang="en-GB" min="09:00" max="17:00" value="12:00"></cs-time-input>`,
      );
      await el.updateComplete;
    ...
    it('an overnight (min > max) range invalidates a value in the excluded window', async () => {
      const el = await fixture<CsTimeInput>(
        html`<cs-time-input lang="en-GB" min="22:00" max="06:00" value="12:00"></cs-time-input>`,
      );
      await el.updateComplete;
    ...
    it('an overnight (min > max) range accepts a value inside the wrapped window', async () => {
      const el = await fixture<CsTimeInput>(
        html`<cs-time-input lang="en-GB" min="22:00" max="06:00" value="23:30"></cs-time-input>`,
      );
      await el.updateComplete;
      expect(el.validity.valid).to.equal(true);
    });
    ...
    it('a step-misaligned value reports stepMismatch', async () => {
      const el = await fixture<CsTimeInput>(
        html`<cs-time-input lang="en-GB" step="900" value="09:07"></cs-time-input>`,
      );
      await el.updateComplete;
    ...
    it('step="any" never reports stepMismatch', async () => {
      const el = await fixture<CsTimeInput>(
        html`<cs-time-input lang="en-GB" step="any" value="09:07:13"></cs-time-input>`,
      );
      await el.updateComplete;
    ...
    it('revalidates when min changes reactively', async () => {
      const el = await fixture<CsTimeInput>(html`<cs-time-input lang="en-GB" value="08:00"></cs-time-input>`);
      await el.updateComplete;
      expect(el.checkValidity()).to.equal(true);
      el.min = '09:00';
      await el.updateComplete;
  ...
  describe('seconds segment composition', () => {
    ...
    it('shows the seconds segment at the withSecondsForStep boundary (step=90, 90 % 60 !== 0)', async () => {
      const el = await fixture<CsTimeInput>(html`<cs-time-input lang="en-GB" step="90"></cs-time-input>`);
      await el.updateComplete;
      const fields = getSegments(el).map((s) => s.dataset.segment);
      expect(fields).to.include('second');
    });
    ...
    it('valueAsDate includes the seconds component', async () => {
      const el = await fixture<CsTimeInput>(
        html`<cs-time-input lang="en-GB" step="1" value="09:30:15"></cs-time-input>`,
      );
      await el.updateComplete;
      const d = el.valueAsDate!;
      expect(d.getHours()).to.equal(9);
  ...
  for (const fixture of fixtures) {
    describe(`SSR + hydration (${fixture.type})`, () => {
      ...
      it('composes the value from a string attribute and survives hydration', async () => {
        const el = await fixture<CsTimeInput>(html`<cs-time-input lang="en-GB" value="14:30"></cs-time-input>`);
        await el.updateComplete;
        expect(el.value).to.equal('14:30');
  ...
  describe('trailing affordance alignment', () => {
    // The expand and clear icons must sit on the same vertical axis as <cs-select>'s, so a
    ...
    function iconCenterFromRight(host: HTMLElement, partSelector: string): number {
      ...
      let icon: Element | null | undefined = part?.querySelector('cs-icon');
      if (!icon && part instanceof HTMLSlotElement) {
        icon = part.assignedElements()[0];
      }
    ...
    it('aligns the expand icon and clear button with <cs-select>', async () => {
      const container = await fixture(html`
        <div>
          <cs-select with-clear value="a"><cs-option value="a">A</cs-option></cs-select>
          <cs-time-input with-clear value="22:03:00"></cs-time-input>
        </div>
      `);
      const select = container.querySelector<HTMLElement & { updateComplete: Promise<unknown> }>('cs-select')!;
      const time = container.querySelector<CsTimeInput>('cs-time-input')!;
      await Promise.all([customElements.whenDefined('cs-select'), customElements.whenDefined('cs-time-input')]);
    ...
    it('keeps the expand icon on the <cs-select> axis with no clear button', async () => {
      ...
      const container = await fixture(html`
        <div>
          <cs-select><cs-option value="a">A</cs-option></cs-select>
          <cs-time-input></cs-time-input>
        </div>
      `);
      const select = container.querySelector<HTMLElement & { updateComplete: Promise<unknown> }>('cs-select')!;
      const time = container.querySelector<CsTimeInput>('cs-time-input')!;
