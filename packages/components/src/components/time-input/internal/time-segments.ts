/**
 * Pure, DOM-free model for the **segmented** time input of `<cs-time-input>`. Plays the same role for time-input as
 ...
 *
 * A `<cs-time-input>` renders hour, minute, optional seconds, and optional AM/PM as discrete spinbutton segments in
...
export function typeTimeDigit(field: TimeField, buffer: string, digit: string, hour12: boolean): TypeDigitResult {
  if (!/^[0-9]$/.test(digit)) {
    return { value: bufferToValue(buffer), buffer, advance: false };
  }
  if (field === 'dayPeriod') {
    return { value: bufferToValue(buffer), buffer, advance: false };
  }
  ...
  if (field === 'hour') {
    if (hour12) {
      return typeNumericSegment(buffer, digit, 1, 12);
    }
...
function typeNumericSegment(buffer: string, digit: string, min: number, max: number): TypeDigitResult {
  ...
  if (buffer === '') {
    ...
    if (d === 0 && min === 0) {
      return { value: 0, buffer: '0', advance: false };
    }
    if (d === 0) {
...
export function isTimeComplete(segments: TimeSegments, opts: WireOptions): boolean {
  ...
  if (opts.withSeconds && segments.second == null) {
    return false;
  }
  if (opts.hour12 && segments.dayPeriod == null) {
    return false;
  }
  return true;
}
...
export function timeSegmentsToWire(segments: TimeSegments, opts: WireOptions): string {
  if (!isTimeComplete(segments, opts)) {
    return '';
  }
...
export function wireToTimeSegments(wire: string | null | undefined, opts: WireOptions): TimeSegments {
  ...
  if (!Number.isFinite(h24) || !Number.isFinite(min)) {
    return empty;
  }
...
export function timeSegmentRules(opts: {
  ...
  return {
    ...
    clear: (group, field) => {
      const segments = opts.getSegments(group);
      if (segments[field as TimeField] == null) {
        return false;
      }
