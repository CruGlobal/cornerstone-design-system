...
/** Part names are kebab-case public API; `dayPeriod` is the one field whose name is not. */
const COLUMN_PARTS: Record<TimeField, string> = {
  hour: 'column-hour',
  minute: 'column-minute',
  second: 'column-second',
  dayPeriod: 'column-day-period',
};

export type CsTimeInputSize = 'xs' | 's' | 'm' | 'l' | 'xl';
export type CsTimeInputPlacement = 'top' | 'top-start' | 'top-end' | 'bottom' | 'bottom-start' | 'bottom-end';
export type CsTimeInputHourFormat = 'auto' | '12' | '24';

let uniqueId = 0;
const generateId = (): string => `cs-time-input-${++uniqueId}`;
 ...
 */
@customElement('cs-time-input')
export default class CsTimeInput extends CornerstoneFormAssociatedElement {
  ...
  @query('.time-input-popup') popup!: CsPopup;
  ...
  @property({ reflect: true }) size: CsTimeInputSize = 'm';
  ...
  /** Only required for SSR. Set to `true` if you're slotting in a `footer` element. */
  ...
  @property({ attribute: 'hour-format', reflect: true }) hourFormat: CsTimeInputHourFormat = 'auto';
  ...
  @property({ reflect: true }) placement: CsTimeInputPlacement = 'bottom-start';
  ...
  async show(): Promise<void> {
    ...
    this.open = true;
    await waitForEvent(this, 'cs-after-show');
  }
  ...
  async hide(): Promise<void> {
    ...
    this.open = false;
    await waitForEvent(this, 'cs-after-hide');
  }
  ...
  get valueAsDate(): Date | null {
    ...
    if (segments.hour == null || segments.minute == null) {
      return null;
    }
    const d = new Date();
  ...
  private handleInputWrapperPointerDown = (event: PointerEvent) => {
    ...
    // <cs-icon> slotted into the toggle button).
  ...
  private handleClearClick = (event: MouseEvent) => {
    event.stopPropagation();
    if (!this._value && isTimeEmpty(this.segments)) {
      return;
    }
    this._value = '';
    this.valueHasChanged = true;
  ...
  private columnItemsFor(field: TimeField): Array<{ label: string; value: number; disabled: boolean }> {
    ...
    if (field === 'hour') {
      ...
      if (this.resolvedHour12) {
        for (let h = 1; h <= 12; h++) {
          items.push({ label: String(h).padStart(2, '0'), value: h, disabled: false });
        }
      } else {
        for (let h = 0; h <= 23; h++) {
          items.push({ label: String(h).padStart(2, '0'), value: h, disabled: false });
  ...
  private scrollColumnsToCurrent() {
    ...
    for (const column of this.shadowRoot.querySelectorAll<HTMLElement>('.column')) {
      ...
      if (item) {
        this.keepItemInView(column, item);
      }
    }
  }
  ...
  private handleColumnKeyDown = (event: KeyboardEvent) => {
    ...
    if (event.key === 'ArrowUp' || event.key === 'ArrowDown' || event.key === 'PageUp' || event.key === 'PageDown') {
      ...
      requestAnimationFrame(() => {
        ...
        if (itemEl) {
          this.keepItemInView(column, itemEl);
        }
      });
      return;
    }
  ...
  render() {
    ...
    const hasFooterSlot = this.hasSlotController.test('footer', 'ssrFooter');
          ...
          >
            ...
            >
              ...
              <button
                ...
                class="expand-button"
                aria-label=${
                  this.open ? this.term('closeTimeInput', 'Close time picker') : this.term('chooseTime', 'Choose time')
                }
              ...
              >
                <slot name="expand-icon" part="expand-icon" class="expand-icon">
                  <cs-icon library="system" name="schedule"></cs-icon>
                </slot>
              </button>
            </div>
  ...
  private renderColumn(field: TimeField): TemplateResult {
    ...
    return html`<div
      part="column ${COLUMN_PARTS[field]}"
