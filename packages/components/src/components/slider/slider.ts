 ...
 * @csspart tooltip - The tooltip, a `<cs-tooltip>` element.
...
export default class CsSlider extends CornerstoneFormAssociatedElement {
  ...
  @query('#tooltip') tooltip: CsTooltip;
  ...
  set value(val: number | null) {
    // A value that isn't a valid floating-point number becomes the default, as it does natively. The
    // previous `Number(val) ?? this.minValue` was wrong twice: `Number()` never returns null so the
    // fallback was unreachable, and `minValue` is a thumb position rather than the lower bound.
  ...
  /** Only required for SSR. Set to `true` if you're slotting in a `reference` element. */
  @property({ attribute: 'ssr-reference', type: Boolean }) ssrReference = false;
  ...
  private showRangeTooltips() {
    ...
    const tooltipMin = this.shadowRoot?.getElementById('tooltip-thumb-min') as CsTooltip;
    const tooltipMax = this.shadowRoot?.getElementById('tooltip-thumb-max') as CsTooltip;
