...
import type CsAnimatedImage from './animated-image.js';

async function loadImage(el: CsAnimatedImage, file: string) {
  const loadingPromise = oneEvent(el, 'cs-load');
  el.src = file;
  await loadingPromise;
}

describe('<cs-animated-image>', () => {
  for (const fixture of fixtures) {
    describe(`with "${fixture.type}" rendering`, () => {
      describe('accessibility', () => {
        it('should be accessible', async () => {
          const el = await fixture<CsAnimatedImage>(html`<cs-animated-image></cs-animated-image>`);
      ...
      describe('properties', () => {
        ...
        it('should reflect the play attribute', async () => {
          const el = await fixture<CsAnimatedImage>(html`<cs-animated-image></cs-animated-image>`);
          await loadImage(el, 'src/internal/test/fixtures/walk.gif');
      ...
      describe('loading', () => {
        const files = ['src/internal/test/fixtures/walk.gif', 'src/internal/test/fixtures/tie.webp'];

        for (const file of files) {
          it(`should load ${file} without errors`, async () => {
            const el = await fixture<CsAnimatedImage>(html`<cs-animated-image></cs-animated-image>`);
            let errorCount = 0;
            oneEvent(el, 'cs-error').then(() => errorCount++);
        ...
        it('should emit cs-error on an invalid URL', async () => {
          const el = await fixture<CsAnimatedImage>(html`<cs-animated-image></cs-animated-image>`);
          const errorPromise = oneEvent(el, 'cs-error');
