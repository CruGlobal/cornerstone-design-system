...
import { CsLoadEvent } from '../../events/load.js';
...
export default class CsIcon extends CornerstoneElement {
  static css = styles;
  ...
  /**
   * The family of icons to choose from. For Material Symbols, this selects the style — valid options are `sharp`
   * (the default), `outlined`, and `rounded`. Custom icon libraries may or may not use this property.
   */
  @property({ reflect: true }) family: string;

  /**
   * The name of the icon's variant. For Material Symbols, valid options are `regular` (the default), which draws the
   * outlined cut, and `fill`, which draws the filled cut. Custom icon libraries may or may not use this property.
   */
  @property({ reflect: true }) variant: string;

  /**
   * The stroke weight of the icon. Material Symbols publishes seven weights from `100` to `700` in steps of 100;
   * values in between snap to the nearest one. Defaults to `400`, which is the weight Cru's brand guidelines
   * specify. Custom icon libraries may or may not use this property.
   */
  @property({ type: Number, reflect: true }) weight = 400;

  /**
   * Sets the icon canvas — the box the icon is centered within. Unset renders as `fixed` (1.25em × 1em); `auto` hugs the
   * icon's width; `square` is 1.25em × 1.25em; `roomy` is 1.5em × 1.5em. Scales with `font-size`.
   */
