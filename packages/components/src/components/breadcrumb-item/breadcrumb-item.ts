...
export default class CsBreadcrumbItem extends CornerstoneElement {
  static css = styles;
  ...
  private setRenderType() {
    const hasDropdown =
      this.defaultSlot.assignedElements({ flatten: true }).filter((i) => i.tagName.toLowerCase() === 'cs-dropdown')
        .length > 0;
...
declare global {
  interface HTMLElementTagNameMap {
    'cs-breadcrumb-item': CsBreadcrumbItem;
  }
}
