...
import { CsHoverEvent } from '../../events/hover.js';
...
export default class CsRating extends CornerstoneFormAssociatedElement {
   ...
   * well with `<cs-icon>` elements.
   */
  @property() getSymbol: (value: number, isSelected: boolean) => string = (_value, isSelected) => {
    return isSelected
      ? '<cs-icon name="star" library="system" variant="fill"></cs-icon>'
      : '<cs-icon name="star" library="system"></cs-icon>';
  };
