...
import type CsKnownDate from './known-date.js';

function getFields(el: CsKnownDate): HTMLInputElement[] {
  return Array.from(el.shadowRoot!.querySelectorAll<HTMLInputElement>('input[part~="field-input"]'));
}

function getField(el: CsKnownDate, field: 'day' | 'month' | 'year'): HTMLInputElement {
...
describe('<cs-known-date>', () => {
  runFormControlBaseTests({ tagName: 'cs-known-date', formValue: { dirtyValue: '2026-01-15' } });

  describe('accessibility', () => {
    it('is accessible with a label', async () => {
      const el = await fixture<CsKnownDate>(
        html`<cs-known-date label="When was your passport issued?" hint="For example, 27 3 2007"></cs-known-date>`,
      );
    ...
    it('is accessible without a label (uses role=group fallback)', async () => {
      const el = await fixture<CsKnownDate>(html`<cs-known-date></cs-known-date>`);
      await el.updateComplete;
      await expect(el).to.be.accessible();
    });
  });
  ...
  describe('rendering & defaults', () => {
    ...
    it('populates fields from value attribute', async () => {
      const el = await fixture<CsKnownDate>(html`<cs-known-date value="2007-03-27"></cs-known-date>`);
      await el.updateComplete;
  ...
  describe('field order by locale', () => {
    function visibleFieldOrder(el: CsKnownDate): string[] {
    ...
    it('renders day/month/year for en-GB', async () => {
      const el = await fixture<CsKnownDate>(html`<cs-known-date lang="en-GB"></cs-known-date>`);
      await el.updateComplete;
      expect(visibleFieldOrder(el)).to.deep.equal(['day', 'month', 'year']);
    });

    it('renders month/day/year for en-US', async () => {
      const el = await fixture<CsKnownDate>(html`<cs-known-date lang="en-US"></cs-known-date>`);
      await el.updateComplete;
      expect(visibleFieldOrder(el)).to.deep.equal(['month', 'day', 'year']);
    });

    it('renders year/month/day for ja-JP', async () => {
      const el = await fixture<CsKnownDate>(html`<cs-known-date lang="ja-JP"></cs-known-date>`);
      await el.updateComplete;
      expect(visibleFieldOrder(el)).to.deep.equal(['year', 'month', 'day']);
    });

    it('the locale property overrides lang', async () => {
      const el = await fixture<CsKnownDate>(html`<cs-known-date lang="en-US" locale="en-GB"></cs-known-date>`);
      await el.updateComplete;
      expect(visibleFieldOrder(el)).to.deep.equal(['day', 'month', 'year']);
    });
  ...
  describe('validation', () => {
    ...
    it('is invalid when required and empty', async () => {
      const el = await fixture<CsKnownDate>(html`<cs-known-date required></cs-known-date>`);
      await el.updateComplete;
    ...
    it('respects min', async () => {
      const el = await fixture<CsKnownDate>(html`<cs-known-date min="2020-01-01" value="2007-03-27"></cs-known-date>`);
      await el.updateComplete;
      expect(el.checkValidity()).to.equal(false);
    });

    it('respects max', async () => {
      const el = await fixture<CsKnownDate>(html`<cs-known-date max="2020-12-31" value="2024-03-27"></cs-known-date>`);
      await el.updateComplete;
      expect(el.checkValidity()).to.equal(false);
    });

    it('emits cs-invalid from reportValidity', async () => {
      const el = await fixture<CsKnownDate>(html`<cs-known-date required></cs-known-date>`);
      await el.updateComplete;
      const event = oneEvent(el, 'cs-invalid');
      el.reportValidity();
      await event;
    });
    ...
    it('enters user-invalid after typing into a field and blurring (real interaction)', async () => {
      const el = await fixture<CsKnownDate>(html`<cs-known-date lang="en-GB" required></cs-known-date>`);
      await el.updateComplete;
      expect(el.customStates.has('user-invalid')).to.equal(false);
  ...
  describe('form association', () => {
    it('submits the canonical ISO value', async () => {
      const form = await fixture<HTMLFormElement>(
        html`<form><cs-known-date name="dob" value="2007-03-27"></cs-known-date></form>`,
      );
      await (form.querySelector('cs-known-date') as CsKnownDate).updateComplete;
      const data = new FormData(form);
      expect(data.get('dob')).to.equal('2007-03-27');
    });

    it('omits the field when value is blank', async () => {
      const form = await fixture<HTMLFormElement>(html`<form><cs-known-date name="dob"></cs-known-date></form>`);
      await (form.querySelector('cs-known-date') as CsKnownDate).updateComplete;
      const data = new FormData(form);
    ...
    it('restores the default value on form reset', async () => {
      ...
      const el = form.querySelector('cs-known-date') as CsKnownDate;
      await el.updateComplete;
      el.value = '2024-01-01';
      await el.updateComplete;
  ...
  describe('autocomplete mapping', () => {
    it('expands bday to per-field tokens', async () => {
      const el = await fixture<CsKnownDate>(html`<cs-known-date autocomplete="bday"></cs-known-date>`);
      await el.updateComplete;
    ...
    it('applies off to all three fields', async () => {
      const el = await fixture<CsKnownDate>(html`<cs-known-date autocomplete="off"></cs-known-date>`);
      await el.updateComplete;
    ...
    it('applies on to all three fields', async () => {
      const el = await fixture<CsKnownDate>(html`<cs-known-date autocomplete="on"></cs-known-date>`);
      await el.updateComplete;
    ...
    it('forwards other (non-directive) families only to the year field', async () => {
      const el = await fixture<CsKnownDate>(html`<cs-known-date autocomplete="section-foo"></cs-known-date>`);
      await el.updateComplete;
      expect(getField(el, 'day').hasAttribute('autocomplete')).to.equal(false);
      expect(getField(el, 'month').hasAttribute('autocomplete')).to.equal(false);
  ...
  describe('disabled / readonly propagation', () => {
    it('disables all three inputs when host is disabled', async () => {
      const el = await fixture<CsKnownDate>(html`<cs-known-date disabled></cs-known-date>`);
      await el.updateComplete;
    ...
    it('makes all three inputs readonly when host is readonly', async () => {
      const el = await fixture<CsKnownDate>(html`<cs-known-date readonly></cs-known-date>`);
      await el.updateComplete;
      for (const f of getFields(el)) {
  ...
  describe('setCustomValidity', () => {
    ...
    it('fires cs-invalid when a custom error fails checkValidity via reportValidity', async () => {
      const el = await fixture<CsKnownDate>(html`<cs-known-date value="2007-03-27"></cs-known-date>`);
      await el.updateComplete;
  ...
  describe('user-invalid reflection on fields', () => {
    ...
    it('never renders an inline error region — validation surfaces through the native popup', async () => {
      ...
      // matching <cs-time-input>. Interacting with an invalid control must not inject any error element.
      const el = await fixture<CsKnownDate>(html`<cs-known-date required></cs-known-date>`);
      await el.updateComplete;
      typeIn(getField(el, 'day'), '15');
      await el.updateComplete;
  ...
  describe('validationTarget anchoring', () => {
    async function compose(day: string, month: string, year: string, lang = 'en-GB') {
      const el = await fixture<CsKnownDate>(html`<cs-known-date lang=${lang}></cs-known-date>`);
      await el.updateComplete;
      typeIn(getField(el, 'day'), day);
      typeIn(getField(el, 'month'), month);
      typeIn(getField(el, 'year'), year);
  ...
  describe('formStateRestoreCallback', () => {
    ...
    it('restores fields and value from a string state after first update', async () => {
      const el = await fixture<CsKnownDate>(html`<cs-known-date name="dob"></cs-known-date>`);
      await el.updateComplete;
      expect(el.value).to.equal('');
    ...
    it('applies a state restored before the first update via pendingValue', async () => {
      ...
      const el = document.createElement('cs-known-date');
  ...
  // isServer validators, uses ssrLabel/ssrHint for slot detection, and stashes value in pendingValue)
  // survives hydration and behaves identically to the client-only path.
  for (const fixture of fixtures) {
    describe(`SSR + hydration ("${fixture.type}")`, () => {
      function getFieldOrder(el: CsKnownDate): string[] {
        return Array.from(el.shadowRoot!.querySelectorAll<HTMLInputElement>('input[data-field]')).map(
          (i) => i.dataset.field as string,
        );
      }
      ...
      it('is accessible with a label and hint', async () => {
        const el = await fixture<CsKnownDate>(
          html`<cs-known-date label="Birthday" hint="For example, 27 3 2007"></cs-known-date>`,
        );
        await el.updateComplete;
        await expect(el).to.be.accessible();
      });
