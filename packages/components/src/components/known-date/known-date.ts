...
export type CsKnownDateSize = 'xs' | 's' | 'm' | 'l' | 'xl';
export type CsKnownDateAppearance = 'filled' | 'outlined' | 'filled-outlined';

const generateId = (): string => uniqueId('cs-known-date-');
...
export default class CsKnownDate extends CornerstoneFormAssociatedElement {
  ...
  @property({ reflect: true }) size: CsKnownDateSize = 'm';

  /** The known date's visual appearance. */
  @property({ reflect: true }) appearance: CsKnownDateAppearance = 'outlined';
  ...
  firstUpdated(changedProperties: PropertyValues<typeof this>) {
    ...
    // than the hidden mirror - matching `<cs-time-input>`.
  ...
  private updateHiddenInput() {
    ...
    // setValue(null) when blank - matches <cs-date-input> so FormData omits the entry instead of
    // submitting `name=`.
  ...
  render() {
    ...
    // `validationTarget` - matching `<cs-time-input>`. We only use `user-invalid` to mark the fields with
      ...
      >
        ${
          hasLabel
            ...
            : html`<div part="fieldset" class="fieldset" role="group" aria-label=${groupAriaLabel}>
                ${groupContent}
              </div>`
        }
