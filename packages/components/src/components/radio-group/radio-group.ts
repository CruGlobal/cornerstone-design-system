...
export default class CsRadioGroup extends CornerstoneFormAssociatedElement {
  ...
  /** The radio group's size. When present, this size will be applied to all `<cs-radio>` items inside. */
  ...
  get validationTarget() {
    ...
    const radio = this.querySelector<CsRadio>(':is(cs-radio):not([disabled])');
    if (!radio) {
      return undefined;
    }
  ...
  formResetCallback(...args: Parameters<CornerstoneFormAssociatedElement['formResetCallback']>) {
    this._value = null;
  ...
  private handleRadioClick = (e: Event) => {
    const clickedRadio = (e.target as HTMLElement).closest<CsRadio>('cs-radio');

    if (
      !clickedRadio ||
      clickedRadio.disabled ||
      (clickedRadio as CsRadio & { forceDisabled?: boolean }).forceDisabled ||
      this.disabled
    ) {
      return;
    }
  ...
  private getAllRadios() {
    return [...this.querySelectorAll<CsRadio>('cs-radio')];
  }
  ...
  private async syncRadioElements() {
    ...
    radios.forEach((radio, index) => {
      ...
      radio.toggleAttribute('data-cs-radio-horizontal', this.orientation !== 'vertical');
      radio.toggleAttribute('data-cs-radio-vertical', this.orientation === 'vertical');
      radio.toggleAttribute('data-cs-radio-first', index === 0);
      radio.toggleAttribute('data-cs-radio-inner', index !== 0 && index !== radios.length - 1);
      radio.toggleAttribute('data-cs-radio-last', index === radios.length - 1);
  ...
  private handleKeyDown(event: KeyboardEvent) {
    ...
    const hasRadioButtons = radios.some((radio) => radio.tagName.toLowerCase() === 'cs-radio-button');
...
CsRadioGroup.disableWarning?.('change-in-update');
