...
import type CsFormatDate from './format-date.js';
    ...
    describe(`with "${fixture.type}" rendering`, () => {
      describe('properties', () => {
        ...
        it('should have correct default values', async () => {
          const el = await fixture<CsFormatDate>(html`<cs-format-date></cs-format-date>`);
      ...
      describe('locale formatting', () => {
        ...
        langs.forEach((lang) => {
          it(`should format correctly for locale: ${lang}`, async () => {
            const el = await fixture<CsFormatDate>(
              html`<cs-format-date .date="${testDate}" lang="${lang}"></cs-format-date>`,
            );
      ...
      describe('weekday property', () => {
        (['narrow', 'short', 'long'] as const).forEach((format) => {
          it(`should format weekday as: ${format}`, async () => {
            const el = await fixture<CsFormatDate>(
              html`<cs-format-date .date="${testDate}" weekday="${format}"></cs-format-date>`,
            );
      ...
      describe('era property', () => {
        (['narrow', 'short', 'long'] as const).forEach((format) => {
          it(`should format era as: ${format}`, async () => {
            const el = await fixture<CsFormatDate>(
              html`<cs-format-date .date="${testDate}" era="${format}"></cs-format-date>`,
            );
      ...
      describe('year property', () => {
        (['numeric', '2-digit'] as const).forEach((format) => {
          it(`should format year as: ${format}`, async () => {
            const el = await fixture<CsFormatDate>(
              html`<cs-format-date .date="${testDate}" year="${format}"></cs-format-date>`,
            );
      ...
      describe('month property', () => {
        (['numeric', '2-digit', 'narrow', 'short', 'long'] as const).forEach((format) => {
          it(`should format month as: ${format}`, async () => {
            const el = await fixture<CsFormatDate>(
              html`<cs-format-date .date="${testDate}" month="${format}"></cs-format-date>`,
            );
      ...
      describe('day property', () => {
        (['numeric', '2-digit'] as const).forEach((format) => {
          it(`should format day as: ${format}`, async () => {
            const el = await fixture<CsFormatDate>(
              html`<cs-format-date .date="${testDate}" day="${format}"></cs-format-date>`,
            );
      ...
      describe('hour property', () => {
        (['numeric', '2-digit'] as const).forEach((format) => {
          it(`should format hour as: ${format}`, async () => {
            const el = await fixture<CsFormatDate>(
              html`<cs-format-date .date="${testDate}" hour="${format}"></cs-format-date>`,
            );
      ...
      describe('minute property', () => {
        (['numeric', '2-digit'] as const).forEach((format) => {
          it(`should format minute as: ${format}`, async () => {
            const el = await fixture<CsFormatDate>(
              html`<cs-format-date .date="${testDate}" minute="${format}"></cs-format-date>`,
            );
      ...
      describe('second property', () => {
        (['numeric', '2-digit'] as const).forEach((format) => {
          it(`should format second as: ${format}`, async () => {
            const el = await fixture<CsFormatDate>(
              html`<cs-format-date .date="${testDate}" second="${format}"></cs-format-date>`,
            );
      ...
      describe('time zone options', () => {
        (['short', 'long'] as const).forEach((format) => {
          it(`should display time zone name as: ${format}`, async () => {
            const el = await fixture<CsFormatDate>(
              html`<cs-format-date .date="${testDate}" time-zone-name="${format}"></cs-format-date>`,
            );
        ...
        ['America/New_York', 'America/Los_Angeles', 'Europe/Zurich'].forEach((tz) => {
          it(`should format in time zone: ${tz}`, async () => {
            const el = await fixture<CsFormatDate>(
              html`<cs-format-date .date="${testDate}" time-zone="${tz}"></cs-format-date>`,
            );
      ...
      describe('hour format property', () => {
        it('should use auto hour format by default', async () => {
          const el = await fixture<CsFormatDate>(
            html`<cs-format-date .date="${testDate}" hour="numeric"></cs-format-date>`,
          );
        ...
        it('should use 12-hour format', async () => {
          const el = await fixture<CsFormatDate>(
            html`<cs-format-date .date="${testDate}" hour="numeric" hour-format="12"></cs-format-date>`,
          );
        ...
        it('should use 24-hour format', async () => {
          const el = await fixture<CsFormatDate>(
            html`<cs-format-date .date="${testDate}" hour="numeric" hour-format="24"></cs-format-date>`,
          );
      ...
      describe('CSS parts and states', () => {
        it('should render a time element with datetime attribute', async () => {
          const el = await fixture<CsFormatDate>(html`<cs-format-date .date="${testDate}"></cs-format-date>`);
      ...
      describe('edge cases', () => {
        it('should handle string date input', async () => {
          ...
          const el = await fixture<CsFormatDate>(html`<cs-format-date date="${isoString}"></cs-format-date>`);
          const expected = new Intl.DateTimeFormat('en-US').format(new Date(isoString));
          expect(el.shadowRoot?.textContent.trim()).to.equal(expected);
        });

        it('should render nothing for invalid date', async () => {
          const el = await fixture<CsFormatDate>(html`<cs-format-date date="not-a-date"></cs-format-date>`);
