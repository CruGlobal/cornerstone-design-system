...
import { CsCompleteEvent } from '../../events/complete.js';
 ...
 */
@customElement('cs-otp-input')
export default class CsOtpInput extends CornerstoneFormAssociatedElement {
  ...
  private syncCursor() {
    if (!this._focused || !this.input || this._activeIndex < 0) {
      return;
    }
  ...
  // Dispatch cs-complete when the value just became fully filled, then submit the form if
  // autosubmit is enabled and no listener canceled the event.
  private maybeDispatchComplete(justCompleted: boolean) {
    if (!justCompleted) {
      return;
    }
    const notCanceled = this.dispatchEvent(new CsCompleteEvent());
    if (this.autosubmit && notCanceled) {
      // Deferred like submitOnEnter — requestSubmit() called synchronously from inside the
      // triggering input event doesn't reliably fire, and this also gives async cs-complete
...
CsOtpInput.disableWarning?.('change-in-update');

declare global {
  interface HTMLElementTagNameMap {
    'cs-otp-input': CsOtpInput;
  }
}
