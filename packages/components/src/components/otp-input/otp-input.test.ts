...
import type CsOtpInput from './otp-input.js';

describe('<cs-otp-input>', () => {
  runFormControlBaseTests({ tagName: 'cs-otp-input', formValue: { dirtyValue: '123456' } });

  for (const fixture of fixtures) {
    describe(`with "${fixture.type}" rendering`, () => {
      describe('defaults', () => {
        it('should have correct default property values', async () => {
          const el = await fixture<CsOtpInput>(html`<cs-otp-input></cs-otp-input>`);
        ...
        it('should render the correct number of segments', async () => {
          const el = await fixture<CsOtpInput>(html`<cs-otp-input length="4"></cs-otp-input>`);
      ...
      describe('label and hint', () => {
        it('should render a label when the label attribute is set', async () => {
          const el = await fixture<CsOtpInput>(html`<cs-otp-input label="Code"></cs-otp-input>`);
          const label = el.shadowRoot!.querySelector('[part~="label"]')!;
          expect(label.textContent.trim()).to.equal('Code');
        });

        it('should render hint text when the hint attribute is set', async () => {
          const el = await fixture<CsOtpInput>(html`<cs-otp-input hint="6 digits"></cs-otp-input>`);
      ...
      describe('value', () => {
        it('should reflect a prefilled value in the segments', async () => {
          const el = await fixture<CsOtpInput>(html`<cs-otp-input value="123"></cs-otp-input>`);
          expect(el.value).to.equal('123');
        });

        it('should filter non-numeric characters when type is numeric', async () => {
          const el = await fixture<CsOtpInput>(html`<cs-otp-input type="numeric"></cs-otp-input>`);
          el.value = 'ab12cd';
          expect(el.value).to.equal('12');
        });

        it('should filter non-alpha characters when type is alpha', async () => {
          const el = await fixture<CsOtpInput>(html`<cs-otp-input type="alpha" length="4"></cs-otp-input>`);
          el.value = 'a1b2';
          expect(el.value).to.equal('ab');
        });

        it('should allow letters and digits when type is alphanumeric', async () => {
          const el = await fixture<CsOtpInput>(html`<cs-otp-input type="alphanumeric" length="4"></cs-otp-input>`);
          el.value = 'a1b2';
          expect(el.value).to.equal('a1b2');
        });

        it('should uppercase when case is upper', async () => {
          const el = await fixture<CsOtpInput>(
            html`<cs-otp-input type="alpha" case="upper" length="3"></cs-otp-input>`,
          );
          el.value = 'abc';
        ...
        it('should lowercase when case is lower', async () => {
          const el = await fixture<CsOtpInput>(
            html`<cs-otp-input type="alpha" case="lower" length="3"></cs-otp-input>`,
          );
          el.value = 'ABC';
          expect(el.value).to.equal('abc');
        });
      ...
      describe('format', () => {
        it('should derive length from the format string', async () => {
          const el = await fixture<CsOtpInput>(html`<cs-otp-input format="### ###"></cs-otp-input>`);
          const segments = el.shadowRoot!.querySelectorAll('[part~="segment"]');
      ...
      describe('events', () => {
        ...
        it('should fire cs-complete once when all segments are filled', async () => {
          const el = await fixture<CsOtpInput>(html`<cs-otp-input length="3"></cs-otp-input>`);
          el.focus();
          const completePromise = oneEvent(el, 'cs-complete');
          await sendKeys({ type: '123' });
          await completePromise;
        });

        it('should not fire cs-complete again if typing continues after completion', async () => {
          const el = await fixture<CsOtpInput>(html`<cs-otp-input length="3"></cs-otp-input>`);
          const spy = sinon.spy();
          el.addEventListener('cs-complete', spy);
          el.value = '12';
          el.focus();
      ...
      describe('form integration', () => {
        it('should submit the value via FormData', async () => {
          const form = await fixture<HTMLFormElement>(html`
            <form>
              <cs-otp-input name="code" value="123456"></cs-otp-input>
            </form>
          `);
        ...
        it('should associate with a form by id via the form attribute', async () => {
          const body = await fixture<HTMLDivElement>(html`
            <div>
              ...
              <cs-otp-input name="code" form="my-form"></cs-otp-input>
            </div>
          `);
          const el = body.querySelector<CsOtpInput>('cs-otp-input')!;
        ...
        it('should reset to defaultValue on form reset', async () => {
          ...
          `);
          const el = form.querySelector<CsOtpInput>('cs-otp-input')!;
          el.value = '999999';
          form.reset();
      ...
      describe('autosubmit', () => {
        it('should submit the form when autosubmit is enabled and the value completes', async () => {
          const form = await fixture<HTMLFormElement>(
            html`<form><cs-otp-input name="code" length="3" autosubmit></cs-otp-input></form>`,
          );
          const el = form.querySelector<CsOtpInput>('cs-otp-input')!;
        ...
        it('should not submit the form when autosubmit is disabled', async () => {
          const form = await fixture<HTMLFormElement>(
            html`<form><cs-otp-input name="code" length="3"></cs-otp-input></form>`,
          );
          const el = form.querySelector<CsOtpInput>('cs-otp-input')!;
          const submitHandler = sinon.spy((event: SubmitEvent) => event.preventDefault());
        ...
        it('should not submit the form when cs-complete is canceled', async () => {
          const form = await fixture<HTMLFormElement>(
            html`<form><cs-otp-input name="code" length="3" autosubmit></cs-otp-input></form>`,
          );
          const el = form.querySelector<CsOtpInput>('cs-otp-input')!;
          ...
          form.addEventListener('submit', submitHandler);
          el.addEventListener('cs-complete', (event) => event.preventDefault());
          el.focus();
          await sendKeys({ type: '123' });
          await el.updateComplete;
      ...
      describe('validation', () => {
        ...
        it('should be invalid when empty and required', async () => {
          const el = await fixture<CsOtpInput>(html`<cs-otp-input required></cs-otp-input>`);
      ...
      describe('custom states', () => {
        ...
        it('should have --filled state when complete', async () => {
          const el = await fixture<CsOtpInput>(html`<cs-otp-input value="123456"></cs-otp-input>`);
          await el.updateComplete;
      ...
      describe('mask', () => {
        it('should not expose the real character as text when mask is true', async () => {
          const el = await fixture<CsOtpInput>(html`<cs-otp-input value="123456" mask></cs-otp-input>`);
        ...
        it('should draw --mask-char via a pseudo-element when mask is true', async () => {
          const el = await fixture<CsOtpInput>(html`<cs-otp-input value="1" mask></cs-otp-input>`);
          const firstSegment = el.shadowRoot!.querySelector('[part~="segment"]')!;
      ...
      describe('appearance', () => {
        it('should draw the active segment focus ring inward for contained appearance', async () => {
          ...
          const el = await fixture<CsOtpInput>(html`<cs-otp-input appearance="contained" value="1"></cs-otp-input>`);
          el.focus();
          await el.updateComplete;
        ...
        it('should keep the default outward focus ring for outlined appearance', async () => {
          const el = await fixture<CsOtpInput>(html`<cs-otp-input appearance="outlined" value="1"></cs-otp-input>`);
          el.focus();
          await el.updateComplete;
          const active = el.shadowRoot!.querySelector('.segment--active') as HTMLElement;
      ...
      describe('selection', () => {
        ...
        async function selectAllAndWait(el: CsOtpInput) {
        ...
        it('should only select entered characters, not the full segment length', async () => {
          const el = await fixture<CsOtpInput>(html`<cs-otp-input value="123" length="6"></cs-otp-input>`);
          el.focus();
          await el.updateComplete;
          await selectAllAndWait(el);
          await sendKeys({ press: 'Backspace' });
        ...
        it('should not delete the value when Backspace is pressed after select() while readonly', async () => {
          const el = await fixture<CsOtpInput>(html`<cs-otp-input value="123456" readonly></cs-otp-input>`);
          el.focus();
          await el.updateComplete;
          await selectAllAndWait(el);
          await sendKeys({ press: 'Backspace' });
      ...
      describe('readonly', () => {
        it('should block keyboard input when readonly', async () => {
          const el = await fixture<CsOtpInput>(html`<cs-otp-input readonly></cs-otp-input>`);
          el.focus();
          await el.updateComplete;
          await sendKeys({ type: '123' });
          await el.updateComplete;
      ...
      describe('with-mask', () => {
        it('should show a hint character in each empty segment', async () => {
          const el = await fixture<CsOtpInput>(html`<cs-otp-input with-mask length="3"></cs-otp-input>`);
          await el.updateComplete;
        ...
        it('should not show the hint in segments that have a value', async () => {
          const el = await fixture<CsOtpInput>(html`<cs-otp-input with-mask value="1" length="3"></cs-otp-input>`);
          await el.updateComplete;
          const hints = el.shadowRoot!.querySelectorAll('.segment--mask-hint');
        ...
        it('should respect a custom --mask-char', async () => {
          const el = await fixture<CsOtpInput>(html`<cs-otp-input with-mask style="--mask-char: '*'"></cs-otp-input>`);
          await el.updateComplete;
      ...
      describe('caret', () => {
        ...
        it('should not draw the caret over a filled segment', async () => {
          ...
          const el = await fixture<CsOtpInput>(html`<cs-otp-input label="Code" value="123456"></cs-otp-input>`);
          el.focus();
          await el.updateComplete;
      ...
      describe('accessibility', () => {
        it('should have aria-describedby pointing to the hint element', async () => {
          const el = await fixture<CsOtpInput>(html`<cs-otp-input hint="Enter your code"></cs-otp-input>`);
          await el.updateComplete;
        ...
        it('should pass accessibility checks', async () => {
          const el = await fixture<CsOtpInput>(
            html`<cs-otp-input label="Verification code" hint="Check your email for a 6-digit code."></cs-otp-input>`,
          );
          await el.updateComplete;
      ...
      describe('enter key', () => {
        it('should submit the containing form when enter is pressed', async () => {
          const form = await fixture<HTMLFormElement>(
            html`<form><cs-otp-input name="code" length="3" value="123"></cs-otp-input></form>`,
          );
          const el = form.querySelector<CsOtpInput>('cs-otp-input')!;
          const submitHandler = sinon.spy((event: SubmitEvent) => event.preventDefault());
      ...
      describe('tab key', () => {
        // Skipped on WebKit, not fixed. The field is a visually-hidden real `<input>` (1×1, opacity 0)
        // reached through `delegatesFocus`, and WebKit does not advance sequential focus out of that
        // combination the way Chromium and Firefox do. Both other engines assert the correct behaviour,
        // and the pattern itself — one real input behind painted segments — is deliberate and load-bearing
        // for input methods and autofill, so bending the component to satisfy one engine's focus walk
        // would cost more than it buys. Revisit if WebKit's behaviour changes.
        (isWebkit ? it.skip : it)('should move focus out of the field when tab is pressed', async () => {
          const container = await fixture<HTMLDivElement>(
            html`<div>
              <cs-otp-input label="Code"></cs-otp-input>
              <button type="button">After</button>
            </div>`,
          );
          const el = container.querySelector<CsOtpInput>('cs-otp-input')!;
      ...
      describe('right-to-left', () => {
        it('should keep segments laid out left-to-right in RTL contexts', async () => {
          const container = await fixture<HTMLDivElement>(
            html`<div dir="rtl"><cs-otp-input label="Code" value="12"></cs-otp-input></div>`,
          );
          const el = container.querySelector<CsOtpInput>('cs-otp-input')!;
          await el.updateComplete;
          const segments = el.shadowRoot!.querySelector('.segments')!;
