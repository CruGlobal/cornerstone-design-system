...
import type CsRandomContent from './random-content.js';

function visibleChildren(el: CsRandomContent): Element[] {
  return Array.from(el.children).filter((c) => !(c as HTMLElement).hidden);
}

function hiddenChildren(el: CsRandomContent): Element[] {
  return Array.from(el.children).filter((c) => (c as HTMLElement).hidden);
}
...
function stubReducedMotion(matches: boolean) {
  return sinon.stub(window, 'matchMedia').callsFake((query: string) => ({
...
describe('<cs-random-content>', () => {
  for (const fixture of fixtures) {
    describe(`with "${fixture.type}" rendering`, () => {
      describe('accessibility', () => {
        it('should be accessible', async () => {
          const el = await fixture<CsRandomContent>(html`
            <cs-random-content>
              <span>A</span>
              <span>B</span>
            </cs-random-content>
      ...
      describe('animation', () => {
        it('plays a fade animation on shown elements when animation="fade"', async () => {
          ...
          try {
            const el = await fixture<CsRandomContent>(html`
              <cs-random-content animation="fade">
                <span>A</span>
                <span>B</span>
                <span>C</span>
              </cs-random-content>
      ...
      describe('margin', () => {
        it('strips the leading and trailing block margins from a single shown element', async () => {
          const el = await fixture<CsRandomContent>(html`
            <cs-random-content mode="sequence">
              <p>A</p>
              <p>B</p>
              <p>C</p>
            </cs-random-content>
        ...
        it('keeps inner margins between shown elements when items > 1', async () => {
          const el = await fixture<CsRandomContent>(html`
            <cs-random-content mode="sequence" items="2">
              <p>A</p>
              <p>B</p>
              <p>C</p>
            </cs-random-content>
      ...
      describe('cs-content-change event', () => {
        it('emits with the shown items and matches the randomize() return value', async () => {
          ...
          el.addEventListener('cs-content-change', (e: Event) => emitted.push((e as CustomEvent).detail.items));
      ...
      describe('reduced motion', () => {
        it('skips the entrance animation but keeps autoplaying', async () => {
          ...
          try {
            const el = await fixture<CsRandomContent>(html`
              <cs-random-content animation="fade" mode="sequence">
                <span>A</span>
                <span>B</span>
                <span>C</span>
              </cs-random-content>
            ...
            const shown = visibleChildren(el)[0] as HTMLElement;
            expect(shown.getAnimations().length).to.equal(0);
            expect(shown.dataset['csAnimation']).to.be.undefined;

            // ...but autoplay still advances (matching <cs-carousel>).
  ...
  describe('initial render', () => {
    it('emits cs-content-change on first render', async () => {
      await customElements.whenDefined('cs-random-content');
      const emitted: Element[][] = [];
      const el = document.createElement('cs-random-content');
      el.addEventListener('cs-content-change', (e: Event) => emitted.push((e as CustomEvent).detail.items));
