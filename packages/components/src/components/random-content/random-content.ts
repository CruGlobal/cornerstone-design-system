...
import { CsContentChangeEvent } from '../../events/random-content-change.js';
 ...
 * @documentation https://cruglobal.github.io/cornerstone-design-system/components/random-content
 * @status experimental
 * @since 0.1
 *
 ...
 *
 * @event {{ items: Element[] }} cs-content-change - Emitted whenever the displayed selection changes,
...
export default class CsRandomContent extends CornerstoneElement {
  ...
  randomize(): Element[] {
    ...
    children.forEach((el) => {
      ...
      // a phantom gap (e.g. a <p> or <cs-callout> whose own margin would otherwise show before the
  ...
  private syncAutoplay() {
    ...
    // autoplay — matching <cs-carousel>, which keeps advancing but without the smooth transition.
...
declare global {
  interface HTMLElementTagNameMap {
    'cs-random-content': CsRandomContent;
  }
}
