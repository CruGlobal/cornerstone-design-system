...
export function deepMerge<T extends Record<string, unknown>, U extends Record<string, unknown>>(
  ...
  const result = deepClone(src) as Record<string, unknown>;
  ...
  // `result` is built by merging T with U key by key, which the type system cannot follow.
  return result as unknown as T & U;
}
