 ...
 * awaiting `customElements.whenDefined()` multiple times. By default, the function waits for all undefined Cornerstone
...
export async function allDefined(options?: Partial<AllDefinedOptions>) {
  const opts: AllDefinedOptions = {
    match: (tagName) => tagName.startsWith('cs-'),
    additionalElements: [],
    root: document,
    ...options,
  };
