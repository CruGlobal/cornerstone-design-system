...
function loadAllFrontMatter(components, contentDir) {
  const cache = new Map();
  ...
  for (const component of components) {
    ...
    const mdPath = path.join(contentDir, 'components', `${componentName}.md`);
...
function generateComponentApiSection(component, frontMatterCache, baseUrl) {
  ...
  const componentSlug = component.tagName.replace(/^cs-/, '');
  ...
  lines.push('');
  lines.push(`**Documentation:** ${baseUrl}/components/${componentSlug}`);
  lines.push('');
  ...
  // Methods
  const methods =
    component.members?.filter((m) => m.kind === 'method' && m.privacy !== 'private' && m.description) || [];
...
function generateLlmsTxt({ components, packageData, frontMatterCache, baseUrl }) {
  ...
  lines.push(
    `
Cornerstone provides a comprehensive set of customizable, accessible web components for building modern
...
Material Symbols is the default icon library, so \`<cs-icon name="...">\` values should reference Material Symbols
icon names, which are snake_case (\`keyboard_arrow_down\`, \`content_copy\`). The \`family\` attribute picks the style
(\`sharp\` by default, or \`outlined\`/\`rounded\`), \`variant="fill"\` picks the filled cut, and \`weight\` takes
100–700. Brand logos live in a separate \`brands\` library.

For AI agents and coding assistants building with Cornerstone: two Agent Skills ship with the package. The
\`cornerstone\` skill is this component reference in progressive, on-demand form; the companion \`cornerstone-design\`
skill covers full-page layout (\`<cs-page>\`), theming, brand color, and visual composition. After installing
\`@cruglobal/cornerstone-components\`, register them with \`npx skills add ./node_modules/@cruglobal/cornerstone-components/dist/unbundled/skills/cornerstone\`
and \`npx skills add ./node_modules/@cruglobal/cornerstone-components/dist/unbundled/skills/cornerstone-design\`. See ${baseUrl}/ai/agent-skills for both.
`.trim(),
  );
  lines.push('');
  ...
  lines.push('');
  lines.push(`For comprehensive documentation, visit ${baseUrl}`);
  lines.push('');
  lines.push(`- [Getting Started](${baseUrl}/): Installation and setup guide`);
  lines.push(`- [Components Overview](${baseUrl}/components): Complete component reference`);
  lines.push(`- [Theming](${baseUrl}/theming-overview): Customization and design tokens`);
  lines.push(`- [Form Controls](${baseUrl}/form-controls): Form integration and validation`);
  lines.push('');
  ...
  for (const component of sortedComponentsList) {
    ...
    lines.push(`- [${title}](${baseUrl}/components/${componentSlug}): ${description || 'No description available.'}`);
  }
  lines.push('');
...
export async function generateLlmsTxtFile(options = {}) {
  ...
  const frontMatterCache = loadAllFrontMatter(components, contentDir);
