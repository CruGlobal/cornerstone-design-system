...
Cornerstone ships a complete, opinionated design system: scales for spacing, type, color, shadow,
...
Most of the principles below aren't unique to Cornerstone — line length, hierarchy via weight and
...
Color in Cornerstone flows through three layers: **palette** (raw hues × tints, `--cs-color-blue-50`),
**semantic** (roles, `--cs-color-brand-fill-loud`), and **theme assignment** (surfaces and text,
`--cs-color-surface-raised`). You almost only ever touch the top two. The semantic layer is what
makes the system re-themeable and dark-mode safe — semantic tokens flip automatically with `cs-light`
/ `cs-dark` because they re-resolve through theme assignments; raw palette tints are frozen at the
...
- **Stay on the scale.** `--cs-color-neutral-90` next to a default surface is fine; a raw hex or a
  custom shade isn't.
...
**The cost:** semantic tokens flip with `cs-light` / `cs-dark` because they re-resolve through theme
assignments. Palette tints don't. So when you reach for a palette tint, you owe a matching
`.cs-dark` override that picks the mirrored step on the scale:

```css
.subtle-band {
  background-color: var(--cs-color-neutral-90);
}
.cs-dark .subtle-band {
  background-color: var(--cs-color-neutral-10);
}
```
...
matches the band it sits on, and a `<cs-callout>` whose body text becomes unreadable after the host
...
`--cs-color-brand-border-normal`, `--cs-color-success-border-normal`, etc. Use them when you want a
...
A `<cs-button appearance="plain">` or any "quiet" control inherits a muted text color tuned for the
...
**In Cornerstone:** on any colored band, secondary actions need a full-contrast on-color text
(`--cs-color-brand-on-loud`, `--cs-color-brand-on-quiet`, etc., matching the band's step) — or use a
filled/outlined appearance with explicit `::part(button)` recolor. Don't leave them on the page-surface
...
Set `background: var(--cs-color-brand-fill-quiet); color: var(--cs-color-brand-on-quiet);` and you
...
darker neutrals don't go flat. **Don't override `--cs-color-neutral-*` with `hsl(0 0% N%)` "true
...
`<cs-callout>` and `<cs-badge>` both accept a slotted `<cs-icon>` — use it whenever the component's
...
### `<cs-page>` layers with z-index, not shadow
...
shadow. The menu and aside columns share elevation with main; a `<cs-card>` _inside_ main is what
the surface tokens raise. If you want a header to cast an elevation shadow as content scrolls under
it, that's on you — add `--cs-shadow-s` or `--cs-shadow-m` to a header-slot background element.

### Picture an overhead light source

`--cs-shadow-s` / `--cs-shadow-m` / `--cs-shadow-l` encode an overhead-light model — raised surfaces
...
where it sits: `--cs-shadow-s` for elements barely off the page (`<cs-card>`), `--cs-shadow-m` for
inline overlays (`<cs-dropdown>`), `--cs-shadow-l` for elements that float in front of everything
(`<cs-popover>`, `<cs-dialog>`, `<cs-drawer>`). When you build custom surfaces, pick the size by
where the element belongs on the z-axis, not by what looks pleasing in isolation.

**Common miss:** the same `--cs-shadow-l` on a card and a dialog — the card reads as floating into
the user's face when it should sit just off the page.

### Surface color does depth without shadows

`--cs-color-surface-raised` / `--cs-color-surface-default` / `--cs-color-surface-lowered` ship as an
...
`margin-block-start: calc(-1 * var(--cs-space-l))`. A card that straddles the boundary between two
...
A `<cs-page>` is itself a hierarchy: the **banner** is the loudest band (announcements), the
...
**In Cornerstone:** put aside content in `cs-color-text-quiet`. Give the footer a recessed feel with
`--cs-color-surface-lowered`. Reserve the `banner` slot for a single loud band when you need user
attention, not as a permanent fixture — `<cs-page>` hides empty slots, so an empty `banner` adds
nothing.

### Hierarchy reads on three axes — let the utilities apply all three

WA's text utilities (`cs-heading-*`, `cs-body-*`, `cs-caption-*`) ship a size, a weight, and a
line-height tuned to work together; pairing them with `cs-color-text-normal` / `-quiet` / `-link`
...
`cs-font-weight-light`, `-normal`, `-semibold`, and `-bold` for explicit overrides — reach for
...
**Common miss:** an 11px metadata line next to an oversized headline. Pair `cs-caption-m` with a
smaller heading instead — the relationship reads at readable sizes.

### Quiet the surroundings to lift the focal element

WA exposes a quiet treatment for nearly every surface — `cs-color-text-quiet` for text,
`<cs-card appearance="plain">` for cards, `--cs-color-surface-lowered` for a sidebar that recedes.
...
The `<cs-button>` attribute combinations encode three importance tiers directly:
...
its trigger secondary; reserve `variant="danger"` for the confirm button inside the `<cs-dialog>` it
opens.

Lay them out with `cs-split` (primary at one end, secondary/cancel at the other) for header bars
and dialog footers, or `cs-cluster cs-justify-content-end` for grouped end-aligned action rows.
...
against busy UI. WA exposes two dials for the compensation: text color (`cs-color-text-quiet` to
dim a heavy element) and border width (`--cs-border-width-s` → `--cs-border-width-m` to amplify a
...
WA's text utilities (`cs-caption-*`, `cs-heading-*`, `cs-body-*`, `cs-visually-hidden`) and HTML
...
that's really a label can be a small `<h2 class="cs-caption-m">`; if the surrounding content speaks
for itself, the heading can stay `cs-visually-hidden`.
...
tiers: render labels with `cs-caption-*` or `cs-color-text-quiet`, render values at the default
...
WA's spacing utilities span twelve steps (`cs-gap-3xs` through `cs-gap-5xl`), but four cover most
of what a layout needs. Start with `cs-gap-l` (24px) between blocks and `cs-gap-m` (16px) between
related items, then tighten with `cs-gap-xs` (8px) or `cs-gap-2xs` (4px) for closely related
...
### Cornerstone's spacing scale is non-linear on purpose
...
**Common miss:** `padding: 14px` because `cs-gap-s` (12px) feels tight and `cs-gap-m` (16px) feels
...
Nest `cs-stack` containers with mismatched `cs-gap-*` to express grouping spatially: a `cs-stack
cs-gap-2xs` for the elements inside a group (label, input), wrapped in a parent `cs-stack cs-gap-l`
...
`<cs-page>`'s grid uses fixed `--menu-width` / `--aside-width` for the sidebars and flexes `main` to
whatever's left — because a sidebar has an optimal width that doesn't benefit from stretching.
`cs-grid` follows the same idea for tile grids: `--min-column-size` defines a content-width minimum,
...
### Full-bleed band, contained content — the canonical `<cs-page>` rhythm
...
`main { padding: 0 }` + `.section { padding-inline: var(--cs-space-xl) }` pattern: the band's color,
...
Set the type size deliberately at each breakpoint by swapping `--cs-font-size-*` tokens
(`--cs-font-size-3xl` for a desktop hero, `--cs-font-size-xl` on mobile) rather than relying on `em`
...
Cornerstone's `--cs-font-size-*` tokens (10/11/12/14/16/20/25/32/41/52/66px) are a 1.125 modular ramp
...
headings with paragraphs and lists — wrap it in `cs-prose` and it handles `h1`–`h6` in `em` along
with the asymmetric spacing rhythm. That's the only case `cs-prose` is for; anywhere you're sizing
a single heading, a card title, or app UI, the size tokens are still the right tool.

### Line length is 45–75 characters

WA gives you three tools for keeping text in a comfortable reading column — `cs-prose`,
`cs-longform-*`, and `cs-text-balance` / `cs-text-pretty`. None are interchangeable; each is scoped
...
- A **multi-element long-form block** (article, docs page, marketing copy that mixes headings,
  paragraphs, and lists): wrap it in `cs-prose`. The utility sets a 65ch reading column and lays in
  asymmetric heading rhythm in a single class. Don't reach for `cs-prose` on a single paragraph,
...
- A **single body element** outside a prose container (a card blurb, an isolated paragraph): the
  `cs-longform-*` text utilities tune font-size and leading for reading, without imposing
  multi-element rhythm.
- **Short blocks where the last line tends to widow** (headings, hero copy, callouts):
  `cs-text-balance` rebalances line breaks across the block; `cs-text-pretty` does the same for
...
Add `cs-align-items-baseline` to any `cs-cluster` or `cs-split` that mixes text sizes — it aligns
...
Use `cs-color-text-link` only on in-prose anchors; in navigation, sidebars, and breadcrumbs, use
`cs-color-text-normal` or `cs-color-text-quiet` instead. The three text-color utilities exist as a
...
WA exposes two cheap surfaces for an accent stripe: a `<cs-card>`'s host border (the same
mechanism that [composition.md § Borders on colored backgrounds](composition.md#borders-on-colored-backgrounds)
uses for re-coloring on a colored band) and `<cs-page>`'s `banner` slot. Either gives you brand
...
```css
.featured-card {
  border-block-start: var(--cs-border-width-l) solid var(--cs-color-brand-fill-loud);
}
```

**Across the entire layout** — `<cs-page>`'s `banner` slot is hidden when empty, so it costs nothing
...
```html
<cs-page>
  <div slot="banner" style="background: var(--cs-color-brand-fill-loud); block-size: var(--cs-space-2xs);"></div>
  …
</cs-page>
```
...
accumulates fastest by replacing them with themed equivalents that pull from your `--cs-*` tokens.
For checkboxes and radios, WA already ships the themed equivalents (`<cs-checkbox>` and `<cs-radio>`)
...
**Icon bullets** — replace native list markers with `<cs-icon>` glyphs:

```html
<ul class="cs-stack cs-gap-xs branded-list">
  <li class="cs-cluster cs-gap-s cs-align-items-start">
    <cs-icon class="branded-list-glyph" name="check"></cs-icon>
...
**Form controls** — `<cs-checkbox>` and `<cs-radio>` are already themed: their selected state, focus
...
```css
blockquote {
  border-inline-start: var(--cs-border-width-l) solid var(--cs-color-brand-fill-loud);
  padding-inline-start: var(--cs-space-m);
...
`<cs-card appearance="filled">` or `<cs-card appearance="plain">` drop the default outline, and
pairing adjacent cards with `--cs-color-surface-raised` vs. `--cs-color-surface-lowered` distinguishes
them without lines. Three WA mechanisms do the separator job that borders usually get reached for:
surface-color shift, subtle shadow (`--cs-shadow-s`), and extra `cs-gap-*` spacing. Borders aren't
the only way to separate two regions, and usually they're the loudest option — try the other three
first.

### Title + action rows want `cs-split`, not `cs-cluster`

A common pattern — section heading on the left, action button on the right — is `cs-split`. It
pushes the first and last children to opposite ends with no inline `style` needed. Reach for
`cs-cluster` for groups that should pack together; `cs-split` for groups that should pull apart.

```html
<div class="cs-split cs-align-items-center">
  <h2 class="cs-heading-m">Members</h2>
  <cs-button variant="brand">Add member</cs-button>
</div>
```

For "fixed icon next to flexible text" (the classic media object), `cs-flank` does the same job:
...
not loud — `--cs-color-brand-fill-quiet` for a brand-tinted band, success-fill-quiet for a soft
...
`linear-gradient(var(--cs-color-brand-fill-quiet), var(--cs-color-brand-fill-normal))`. Two hues
within ~30° of each other read as natural light; further apart looks like a sticker.

### Common components can do uncommon things

WA's component slots are wider than the canonical use cases suggest. `<cs-dropdown>` contents are
whatever you slot in — they don't have to be a `<cs-menu>`; columns, supporting text, or icons all
work. `<cs-card>` containing a `<cs-radio>` is a valid selectable-card pattern. `<cs-tab-group>`
...
The empty-state pattern is a centered `cs-stack cs-gap-l cs-align-items-center cs-text-center` block:
a large `<cs-icon>`, an `<h2 class="cs-heading-l">`, supporting copy in `cs-color-text-quiet`, and a
`<cs-button variant="brand">` CTA. Build it from the moment the feature exists, not after — empty
...
```html
<div class="cs-stack cs-gap-l cs-align-items-center cs-text-center empty-state">
  <cs-icon class="empty-state-glyph" name="inbox"></cs-icon>
  <h2 class="cs-heading-l">No invoices yet</h2>
  <p class="cs-color-text-quiet">Once you send your first invoice, you'll see it here.</p>
  <cs-button variant="brand">Create invoice</cs-button>
</div>
...
`cs-frame` (with one of `cs-frame:square`, `cs-frame:landscape`, `cs-frame:portrait` as a modifier)
crops user-uploaded images to a consistent aspect ratio — the layout stays put regardless of what
the user uploads. For people, prefer `<cs-avatar>` instead; it renders a clean initials/icon
...
`<cs-skeleton>` (content-shaped placeholders), `<cs-spinner>` (tight inline busy state), and
`<cs-button loading>` (action-button waits) cover the three places where a UI pauses. Pick the one
