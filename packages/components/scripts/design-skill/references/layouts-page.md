# Full-page layouts with `<cs-page>`

You are here because STEP 0 determined you're building a **full page, app shell, or site layout**. In
this branch, `<cs-page>` is **required** and the rules below are absolute. Do not hand-roll a full-page
grid; `<cs-page>` exists precisely so you don't have to.

**`<cs-page>` fully owns the page layout.** It is the grid, the sticky regions, the responsive
...
`<cs-page>` is a grid of named regions. Picture **5 stacked rows**, where the middle row splits into
**3 columns**:
...
This is the single most important thing to understand about `<cs-page>`, and the most common source of
...
- On **mobile** (below the breakpoint), that _same_ content is moved into a `<cs-drawer>`, toggled by a
  hamburger button the component shows for you.

You write the nav **once**, in `slot="navigation"`, and `<cs-page>` handles both views. The same is true
...
- **For a single nav, write it once in `slot="navigation"` — don't also copy it into `header`.** One
  ...
  left sidebar. That double-nav-on-desktop is the most common `<cs-page>` mistake.
...
- **Don't write media queries for nav, the sidebar, or the toggle.** The desktop↔mobile switch is driven
  by `<cs-page>`'s own `view` state, not by your CSS. Reserve media queries for genuinely page-specific
...
3. Hide the header links on mobile: `cs-page[view='mobile'] .header-nav { display: none }`.
4. Hide the desktop sidebar on desktop: `cs-page[view='desktop']::part(navigation) { display: none }`.
...
```html
<cs-page>
  <header slot="header" class="cs-split section">
    <strong>My Brand</strong>
    <!-- Desktop links — shown in the header, hidden on mobile via step 3 -->
    <nav class="header-nav cs-cluster cs-gap-l">
      <a href="#features">Features</a>
      <a href="#pricing">Pricing</a>
      <a href="#faq">FAQ</a>
      <cs-button variant="brand">Get started</cs-button>
    </nav>
    <!-- Your OWN toggle, last child so cs-split right-aligns it. Mobile-only; opens the drawer.
         Supplying this suppresses <cs-page>'s built-in hamburger (see note below). -->
    <cs-button data-toggle-nav appearance="plain" class="cs-mobile-only">
      <cs-icon name="menu" label="Open menu"></cs-icon>
    </cs-button>
  </header>
...
You do **not** need your own `<cs-drawer>` or any media queries for this — the drawer comes from
`<cs-page>`. **You should, however, supply your own `data-toggle-nav` button inside the `header`** (as
shown above), so the hamburger is placed and styled _with_ your header bar. Adding any
`[data-toggle-nav]` element automatically suppresses `<cs-page>`'s built-in hamburger. If you omit your
...
`.cs-mobile-only` class shows your toggle only below `mobile-breakpoint`, so the desktop header stays
...
## Everything below is `<cs-page>`-only

The capabilities in this file **only work on elements inside a `<cs-page>`**. Outside one, they are
inert; they do nothing, silently. Do not lift them into a section, widget, or any layout that isn't a
`<cs-page>`. This includes:

- The **slots** (`slot="header"`, `slot="navigation"`, `slot="aside"`, etc.). `slot` only matters on a direct child of `<cs-page>`.
- The **`view='mobile'` / `view='desktop'` state** and the CSS that keys off it.
- **`.cs-desktop-only` / `.cs-mobile-only`**. These are not general responsive utilities; they work _only_ via `<cs-page>`'s `view` selector. Outside `<cs-page>` they do nothing; use a CSS media query instead.
- **`data-toggle-nav`**. Only `<cs-page>` listens for it.
...
1. **Reset `html` and `body`.** `<cs-page>` injects this reset itself (via `:has(cs-page)`), but include
...
3. **Set `main { padding: 0 }` for full-bleed pages — `<cs-page>` pads `main` by default.** `<cs-page>` pads the main content area, which insets
   ...
   .section {
     padding-inline: var(--cs-space-xl); /* or wrap content in a centered max-width container */
   }
   ```
...
5. **Only set a fixed `--menu-width` when you actually render a desktop sidebar; reset it on mobile.**
   ...
   cs-page[view='mobile'] [slot='aside'] {
     display: none; /* aside has no drawer; hide it explicitly on mobile */
   }
   ```

6. **Let `<cs-page>` own the nav — don't toggle it, don't media-query it.** Put nav in
   ...
   toggle, add `data-toggle-nav` to any element inside `<cs-page>` (this auto-hides the default
...
7. **Close the drawer when a nav link is tapped.** The mobile navigation is a `<cs-drawer>`, so add
...
8. **Custom elements never self-close.** `<cs-button></cs-button>`, not `<cs-button />`.
...
9. **`view` is read-only; never set it.** The component sets `view='mobile'` / `view='desktop'` itself
   ...
   (`cs-page[view='mobile'] { … }`). Don't gate critical initial rendering on it.

---

## Canonical example — landing page

A hero-driven marketing/landing page. Nav goes in `slot="navigation"` **once**; `<cs-page>` shows it as a
sidebar on desktop and as a drawer (with a hamburger) on mobile. **No hand-rolled `<cs-drawer>`, no
...
> way to duplicate nav, and it still lets `<cs-page>` provide the drawer and hamburger.
...
<cs-page mobile-breakpoint="920">
  ...
  <nav slot="subheader" class="cs-cluster cs-gap-xs">
    <cs-button data-toggle-nav appearance="plain" class="cs-mobile-only">
      <cs-icon name="menu" label="Menu"></cs-icon>
    </cs-button>
    <cs-breadcrumb>
      <cs-breadcrumb-item href="/">Home</cs-breadcrumb-item>
      <cs-breadcrumb-item>Layouts</cs-breadcrumb-item>
    </cs-breadcrumb>
  </nav>
  ...
  <aside slot="aside" class="cs-desktop-only cs-stack cs-gap-m">
    <strong>On this page</strong>
    <ul class="cs-stack cs-gap-2xs">
...
| Expect `<cs-page>` to emit `<main>`/`<header>`                                                                   | Slot in your own semantic elements                                                                                                                                                  |
...
| Hand-roll your own `<cs-drawer>` + toggle button for the mobile menu                                             | Put nav in `slot="navigation"`; `<cs-page>` provides the drawer and hamburger automatically                                                                                         |
...
| Pair `data-toggle-nav` with your own `<cs-drawer>` (it toggles the `navigation` drawer, not yours → dead button) | Use the `navigation` slot's built-in drawer; `data-toggle-nav` only ever controls that one                                                                                          |
| Set `--menu-width: 16rem` and leave it on mobile                                                                 | Reset widths to `auto` under `cs-page[view='mobile']`                                                                                                                               |
| Nav links that leave the drawer open after a tap                                                                 | Add `data-drawer="close"` to navigation links                                                                                                                                       |
| Expect `aside` to disappear on mobile on its own                                                                 | `aside` has no drawer; hide it (`.cs-desktop-only` or `display: none`)                                                                                                              |
| Try to set `view="mobile"` yourself                                                                              | `view` is read-only; the component sets it. Only read it in CSS                                                                                                                     |
| Hand-roll a `display: grid` page shell                                                                           | Use `<cs-page>`; it already is the grid                                                                                                                                             |
| Nest `<cs-page>` inside a section or another page                                                                | One `<cs-page>` per page, at the top level                                                                                                                                          |
| Hardcode header colors with hex                                                                                  | Use `--cs-color-surface-*` / semantic tokens                                                                                                                                        |
| `<cs-button />` (self-closing)                                                                                   | `<cs-button></cs-button>`                                                                                                                                                           |

## `<cs-page>` checklist

Before calling a `<cs-page>` layout done, walk this **`<cs-page>`-specific** structural pass. This sits
...
- [ ] **Right sidebar decision.** Landing page: nav in `slot="navigation"` **once** (auto sidebar on desktop, drawer on mobile) — or, if you want header-bar nav with no desktop sidebar, the header-on-desktop / drawer-on-mobile recipe above with `--menu-width` left at `auto`. App shell / docs: `navigation` slot with `--menu-width` set, reset to `auto` under `cs-page[view='mobile']`. Either way, don't hand-roll a `<cs-drawer>`.
- [ ] **`html, body` reset** is in place (or you're using native styles) — otherwise gaps appear around the page.
- [ ] **Your own semantic elements are slotted** — `<header>`, `<main>`, `<footer>`, `<nav>`, `<aside>`. `<cs-page>` emits none.
...
- [ ] **Sidebar widths reset on mobile.** `--menu-width` and `--aside-width` go to `auto` under `cs-page[view='mobile']` so they don't leak; the `aside` is hidden (`display: none`) on mobile since it has no auto-drawer.
...
- [ ] **`view` is read-only.** Read it in CSS (`cs-page[view='mobile']`); never set it as an attribute.
- [ ] **One `<cs-page>` per page**, at the top level — never nested inside a section or another `<cs-page>`.
...
```html
<cs-page disable-sticky="aside"> … </cs-page>
```
...
`<cs-page>` is SSR-safe, but a few defaults exist because the mobile drawer needs JavaScript and can't run
...
- **Prevent hydration layout shift by pre-setting the height custom properties.** `--banner-height`,
  ...
  set them on `<cs-page>` so sticky offsets are correct before JS runs. See
...
| `aside`                  | Right sidebar (e.g. table of contents). Sticky. **No auto-drawer — hide it on mobile yourself** (`cs-page[view='mobile'] [slot='aside'] { display: none }`) or it overlaps. |
...
Style internal regions with `::part()` from outside the component (e.g. `cs-page::part(header) { … }`).
...
| `navigation-toggle`      | The default hamburger `<cs-button>`.                                                                          |
| `navigation-toggle-icon` | The default hamburger `<cs-icon>`.                                                                            |
...
| `drawer`                 | The mobile navigation `<cs-drawer>`. (Drawer internals are also exposed via `drawer__*` parts.)              |
