# Cornerstone design skill — source & maintenance

This directory is the **hand-maintained source** for the `cornerstone-design` agent skill, the skill
that teaches AI tools _how to design_ with Cornerstone (layouts with `<cs-page>`, theming with
`--cs-*` tokens, and visual composition). It complements the auto-generated `cornerstone` skill, which
...
scripts/design-skill/                  ← EDIT THESE (source of truth, in git)
  ...
  references/
    layouts-page.md                      full-page layouts with <cs-page> + cs-page checklist
...
**Never edit `dist/unbundled/skills/cornerstone-design/` directly**; every build overwrites it from this source.
...
- **Broken `<cs-*>` references** anywhere in the skills — must exist in either `src/components/`
...
| The `<cs-page>` slots, attributes, or CSS custom properties                                 | `references/layouts-page.md` API tables (verifier catches typos/drift)   |
...
to the docs site and the `cornerstone` skill.
...
- The first thing `SKILL.md` does is the **STEP 0 layout decision gate** (full page → `<cs-page>`;
...
- **Two self-check passes, scoped distinctly.** `SKILL.md`'s **Final Pass** is the **structural**
  ...
  **visual quality** pass (spacing rhythm, hierarchy, contrast, surface choices). `layouts-page.md`
  has its own `<cs-page>`-specific structural checklist. Don't let items overlap across the three.
...
- **Companion utilities are part of the layout story.** When introducing a layout pattern, name the
  applicable companion utility (`cs-align-items-*`, `cs-justify-content-*`, `cs-text-*`, `cs-size-*`,
  `cs-color-text-*`, `cs-visually-hidden`) so agents don't fall back to inline styles.
