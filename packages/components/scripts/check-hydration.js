 ...
 * `lit-hydration-error` event (dispatched by CornerstoneElement when a component
...
async function getComponentUrls(options = {}) {
  ...
  return files
    ...
    .sort()
    .map((name) => `${BASE_URL}/components/${name}/?ssr=true`);
}
...
async function waitForComponentsLoaded(page) {
  await page.evaluate(async () => {
    ...
    //    CornerstoneElement dispatches lit-hydration-error) must have run for
    ...
    for (let i = 0; i < 10; i++) {
      ...
      if (!els.some((el) => el.isUpdatePending)) {
        break;
      }
    }
  });
...
async function checkUrl(context, url) {
  ...
  // Race-proof safety net: CornerstoneElement re-throws after dispatching
...
export async function check(options = {}) {
  ...
  await context.addCookies([{ name: 'cornerstone_ssr', value: 'true', url: BASE_URL }]);
  await context.addInitScript(() => {
    window.__hydrationErrors = [];

    // Primary channel: the event CornerstoneElement dispatches before re-throwing.
